...
under the induced random partition distribution
([**blackwell1973?**](#ref-blackwell1973); [Neal 2000](#ref-neal2000)).
In this view, observation $`i`$ joins an existing cluster with
probability proportional to its current size, and starts a new cluster
with probability proportional to $`\kappa`$, coupled with the base
measure $`H_0`$.
