...
to an Amoroso mixture bulk. Let \\F\_{mix}\\ denote the Amoroso mixture
CDF. The spliced CDF is \\F(x)=F\_{mix}(x)\\ for \\x\<threshold\\ and
\\F(x)=F\_{mix}(threshold) + \left\\1-F\_{mix}(threshold)\right\\G(x)\\
for \\x\ge threshold\\, where \\G\\ is the GPD CDF for exceedances above
`threshold`.
