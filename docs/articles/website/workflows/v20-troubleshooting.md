...
**Cause**: You called
[`fitted()`](https://rdrr.io/r/stats/fitted.values.html) or
[`residuals()`](https://rdrr.io/r/stats/residuals.html) on an
unconditional (no-covariate) fit.

**Fix**: [`fitted()`](https://rdrr.io/r/stats/fitted.values.html) and
[`residuals()`](https://rdrr.io/r/stats/residuals.html) are not
supported for unconditional models. Use
[`predict()`](https://rdrr.io/r/stats/predict.html) for predictions and
posterior summaries (e.g. `predict(fit, type = "mean")`,
`predict(fit, type = "quantile", index = 0.5)`).
...
when using R â‰¥ 4.0. Otherwise, move the project to a local non-synced
path or take ownership of the OneDrive folder (rightâ€‘click â†’
Properties â†’ Security â†’ Advanced â†’ Take ownership).
