...
testthat::test_that("All nimble::nimbleFunction-defined objects in R/ compile", {
  ...
  # Source everything into a clean env, so we compile what the files define.
  src_env <- new.env(parent = baseenv())
  for (ff in r_files) {
    sys.source(ff, envir = src_env, keep.source = TRUE)
  }
  ...
  for (i in seq_len(nrow(defs))) {
    ...
    if (!exists(nm, envir = src_env, inherits = FALSE)) {
      failures[[length(failures) + 1]] <- paste0(
        "- ", nm, " @ ", where, " :: object not found after sourcing R/"
      )
      next
    }

    obj <- get(nm, envir = src_env, inherits = FALSE)
  ...
  testthat::succeed(paste0("compileNimble succeeded for ", length(successes), " objects."))
})
