...
testthat::test_that("package loads and key S3 methods exist", {
  ns <- asNamespace("DPmixGPD")

  # exported constructors / fitters
  for (fn in c("fit.dpm", "fit.dpmgpd", "fit.TE", "ate", "qte", "cate", "cqte")) {
    testthat::expect_true(exists(fn, envir = ns, inherits = FALSE), info = fn)
  }

  # we do NOT export a predict() generic; we register S3 methods under stats::predict
  testthat::expect_true(!exists("predict", envir = ns, inherits = FALSE))
  testthat::expect_true(!is.null(utils::getS3method("predict", "mixgpd_fit", optional = TRUE)))
  testthat::expect_true(!is.null(utils::getS3method("predict", "mixgpd_te_fit", optional = TRUE)))
})
