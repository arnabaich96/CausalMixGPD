.tiny_mcmc <- function() list(n_iter = 120L, burn_in = 40L, thin = 2L, chains = 1L)

testthat::test_that("plot methods run without error (smoke)", {
  set.seed(7)
  dat <- data.frame(y = stats::rgamma(140, shape = 2, scale = 1))
  ...
  testthat::expect_error(graphics::plot(fit0, type = "density"), NA)
  testthat::expect_error(graphics::plot(fit0), NA)
})
...
testthat::test_that("plot.mixgpd_te_fit: default ATE plot is stable", {
  ...
  set.seed(1)
    fit_te <- .get_fit_te_sb_gamma_uncond(K = 5, n = 200, seed = 1)
