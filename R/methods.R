...
#'   \item{kernel}{Kernel family used}
#'   \item{n_train}{Number of training observations}
#'   \item{labels_new}{(if newdata) Integer vector of predicted cluster labels}
#'   \item{probs_new}{(if newdata) N_new x K matrix of predicted membership probabilities}
...
allocation.mixgpd_fit <- function(object, method = c("dahl"), show_progress = TRUE, ...) {
  ...
  GPD <- isTRUE(meta$GPD)
  kernel <- meta$kernel %||% "unknown"
  has_X <- isTRUE(meta$has_X)

  # Guard: backend must be CRP or spliced
  ...
  y_train <- object$data$y %||% object$y
  if (is.null(y_train)) {
    stop("Training response data not found in fit object.", call. = FALSE)
  }
  X_train <- if (has_X) (object$data$X %||% object$X) else NULL
  n_train <- length(y_train)

  # Parse newdata from ...
  dots <- list(...)
  newdata <- dots$newdata %||% NULL
  has_newdata <- !is.null(newdata)

  # If newdata provided, validate
  if (has_newdata) {
    ...
    if (!"y" %in% names(newdata)) {
      stop("newdata must contain a 'y' column (response variable).", call. = FALSE)
    }
    y_new <- newdata$y
    n_new <- length(y_new)

    # If conditional model, check for covariates
    if (has_X) {
      X_cols <- setdiff(names(newdata), "y")
      if (length(X_cols) == 0) {
        stop("Conditional model requires covariates in newdata.", call. = FALSE)
      }
      X_new <- newdata[, X_cols, drop = FALSE]
    } else {
      X_new <- NULL
    }
  }
  ...
  # ============================================================================
  # Newdata allocation (if provided)
  # ============================================================================
  ...
  if (has_newdata) {
    ...
    for (i in seq_len(n_new)) {
      ...
      distances <- abs(y_new[i] - cluster_means)
      # Convert to probabilities (inverse distance weighting)
      weights <- 1 / (distances + 1e-10)
      probs_new[i, ] <- weights / sum(weights)
    }

    # Assign labels as argmax
    labels_new <- apply(probs_new, 1, which.max)
  }
  ...
  # Add newdata components if present
  if (has_newdata) {
    result$n_new <- n_new
    result$labels_new <- labels_new
    result$probs_new <- probs_new
    result$y_new <- y_new
    result$X_new <- X_new
  } else {
    result$n_new <- NULL
...
#'
#' @param x An object of class \code{mixgpd_allocation}.
#' @param return Character; what to display: \code{"label"} (default) shows cluster
#'   sizes, \code{"prob"} shows certainty summaries.
#' @param ... Additional arguments (ignored).
#' @return Invisibly returns the input object.
#' @export
print.mixgpd_allocation <- function(x, return = c("label", "prob"), ...) {
  return <- match.arg(return)

  # Header
  cat("Cluster Allocation (", x$method, " method)\n", sep = "")
  cat("Backend:", .backend_label(x$backend), "\n")
  cat("Kernel:", .kernel_label(x$kernel), "\n")
  cat("GPD tail:", if (x$GPD) "TRUE" else "FALSE", "\n")
  cat("Components:", x$components, "\n")
  cat("Training n:", x$n_train, "\n")
  if (!is.null(x$n_new)) {
    cat("New data n:", x$n_new, "\n")
  }
  cat("\n")
  ...
  if (return == "label") {
    ...
    print(table(Cluster = x$labels_train))
    cat("\n")

    if (!is.null(x$labels_new)) {
      cat("New data cluster sizes:\n")
      print(table(Cluster = x$labels_new))
      cat("\n")
    }
  } else if (return == "prob") {
    # Certainty summaries
    max_prob_train <- apply(x$probs_train, 1, max)
    cat("Training allocation certainty (max probability):\n")
    print(summary(max_prob_train))
    cat("Fraction with prob > 0.9:", mean(max_prob_train > 0.9), "\n")
    cat("Fraction with prob > 0.8:", mean(max_prob_train > 0.8), "\n")
    cat("Fraction with prob > 0.7:", mean(max_prob_train > 0.7), "\n")
    cat("\n")

    if (!is.null(x$probs_new)) {
      max_prob_new <- apply(x$probs_new, 1, max)
      cat("New data allocation certainty (max probability):\n")
      print(summary(max_prob_new))
      cat("Fraction with prob > 0.9:", mean(max_prob_new > 0.9), "\n")
      cat("Fraction with prob > 0.8:", mean(max_prob_new > 0.8), "\n")
      cat("Fraction with prob > 0.7:", mean(max_prob_new > 0.7), "\n")
      cat("\n")
    }
  }
...
#'
#' @param object An object of class \code{mixgpd_allocation}.
#' @param ... Additional arguments (ignored).
#' @return An object of class \code{summary.mixgpd_allocation} containing detailed
#'   summaries of cluster labels and probabilities.
#' @export
summary.mixgpd_allocation <- function(object, ...) {
  # Compute label summaries
  cluster_sizes_train <- table(Cluster = object$labels_train)
  cluster_sizes_new <- if (!is.null(object$labels_new)) {
    table(Cluster = object$labels_new)
  } else {
    NULL
  }

  # Compute probability summaries
  max_prob_train <- apply(object$probs_train, 1, max)
  certainty_train <- list(
    summary = summary(max_prob_train),
    above_0.9 = mean(max_prob_train > 0.9),
    above_0.8 = mean(max_prob_train > 0.8),
    above_0.7 = mean(max_prob_train > 0.7)
  )

  certainty_new <- NULL
  if (!is.null(object$probs_new)) {
    max_prob_new <- apply(object$probs_new, 1, max)
    certainty_new <- list(
      summary = summary(max_prob_new),
      above_0.9 = mean(max_prob_new > 0.9),
      above_0.8 = mean(max_prob_new > 0.8),
      above_0.7 = mean(max_prob_new > 0.7)
    )
  }
  ...
  result <- list(
    label = list(
      ...
      cluster_sizes_new = cluster_sizes_new
    ),
    prob = list(
      certainty_train = certainty_train,
      certainty_new = certainty_new
    ),
    meta = list(
  ...
  class(result) <- "summary.mixgpd_allocation"
  return(result)
}

#' Print summary of cluster allocation
#'
#' @param x An object of class \code{summary.mixgpd_allocation}.
#' @param ... Additional arguments (ignored).
#' @return Invisibly returns the input object.
#' @export
print.summary.mixgpd_allocation <- function(x, ...) {
  cat("Cluster Allocation Summary\n")
  cat("==========================\n\n")

  # Meta information
  cat("Model:\n")
  cat("  Backend:", .backend_label(x$meta$backend), "\n")
  cat("  Kernel:", .kernel_label(x$meta$kernel), "\n")
  cat("  GPD tail:", if (x$meta$GPD) "TRUE" else "FALSE", "\n")
  cat("  Method:", x$meta$method, "\n")
  cat("  Training n:", x$meta$n_train, "\n")
  if (!is.null(x$meta$n_new)) {
    cat("  New data n:", x$meta$n_new, "\n")
  }
  cat("\n")
  ...
  print(x$label$cluster_sizes_train)

  if (!is.null(x$label$cluster_sizes_new)) {
    cat("\nNew data cluster sizes:\n")
    print(x$label$cluster_sizes_new)
  }
  cat("\n")

  # Probability summaries
  cat("Allocation Certainty (Training):\n")
  cat("  Max probability:\n")
  print(x$prob$certainty_train$summary)
  cat("  Fraction > 0.9:", sprintf("%.3f", x$prob$certainty_train$above_0.9), "\n")
  cat("  Fraction > 0.8:", sprintf("%.3f", x$prob$certainty_train$above_0.8), "\n")
  cat("  Fraction > 0.7:", sprintf("%.3f", x$prob$certainty_train$above_0.7), "\n")

  if (!is.null(x$prob$certainty_new)) {
    cat("\nAllocation Certainty (New Data):\n")
    cat("  Max probability:\n")
    print(x$prob$certainty_new$summary)
    cat("  Fraction > 0.9:", sprintf("%.3f", x$prob$certainty_new$above_0.9), "\n")
    cat("  Fraction > 0.8:", sprintf("%.3f", x$prob$certainty_new$above_0.8), "\n")
    cat("  Fraction > 0.7:", sprintf("%.3f", x$prob$certainty_new$above_0.7), "\n")
  }
...
#'
#' Creates diagnostic plots for cluster allocations, including cluster sizes,
#' response distributions by cluster, and allocation certainty.
#'
#' @param x An object of class \code{mixgpd_allocation}.
#' @param overlay Logical; if \code{TRUE} (default) and new data is present,
