#' Internal helper distributions for custom kernels
#'
...
dcauchy <- nimble::nimbleFunction(
  run = function(x = double(0),
    ...
    z <- (x - location) / scale
    val <- (1.0 / (3.141592653589793 * scale)) * (1.0 / (1.0 + z*z))
    if (val < eps) val <- eps
...
dlaplace <- nimble::nimbleFunction(
  run = function(x = double(0),
                 location = double(0),
                 scale = double(0),
                 log = integer(0, default = 0)) {
...
plaplace <- nimble::nimbleFunction(
...
rlaplace <- nimble::nimbleFunction(
...
dlaplace_nimble <- dlaplace
plaplace_nimble <- plaplace
rlaplace_nimble <- rlaplace
dinvgauss_nimble <- dinvgauss
pinvgauss_nimble <- pinvgauss
rinvgauss_nimble <- rinvgauss
dpareto_nimble <- dpareto
ppareto_nimble <- ppareto
rpareto_nimble <- rpareto
dcauchy_nimble <- dcauchy
