...
# draws must contain w[j], shape[j], beta_scale[j,q]
#' @keywords internal
.extract_sb_gamma_reg_params <- function(draws, K, p, renormalize_weights = TRUE) {
  ...
  if (length(w_cols) < K || length(shape_cols) < K) {
    stop("Regression prediction requires draws for w[j] and shape[j].", call. = FALSE)
  }
  ...
  # ---- beta_scale handling ----
  # Preferred: beta_scale[j,q] with K*p columns.
  # Fallback: intercept-only beta_scale[1,j] with K columns.
  Beta <- array(NA_real_, dim = c(nrow(draws), K, p))

  # full regression pattern beta_scale[j,q]
  if (length(beta_cols) >= K * p) {
    ...
    for (idx in seq_along(beta_cols)) {
      j <- jq[idx, "j"]; q <- jq[idx, "q"]
      if (j <= K && q <= p) {
        Beta[, j, q] <- draws[, beta_cols[idx]]
      }
    }

    # If some Beta entries are still NA (e.g., extra columns but missing some j,q),
    # treat missing coefficients as 0 to avoid crashing downstream.
    Beta[is.na(Beta)] <- 0
  ...
  if (length(beta_int_cols) >= K && p >= 1L) {
    ...
    # fill only first column (which is the intercept column in your X matrix)
