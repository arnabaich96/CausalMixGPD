...
#'   }
#' @return A fitted object of class \code{"mixgpd_fit_dpmgpd_sb"}.
#' @examples
#' \dontrun{
#' fit <- dpmgpd.sb(y ~ 1, data = my_data, kernel = "gamma", num_comp = 3,
#'                  mcmc = list(niter = 2000, nburn = 1000),
#'                  tail_ctrl = list(u_prob = 0.9))
#' plot(fit)
#' }
#' @export
dpmgpd.sb <- function(formula, data, kernel,
                      num_comp = 3,
  ...
  # Fit bulk model
  bulk_fit <- dpm.sb(formula = formula, data = data, kernel = kernel,
  ...
  fit$spec <- spec
  fit$draws <- draws
  class(fit) <- "mixgpd_fit_dpmgpd_sb"
  fit
}

#' @export
print.mixgpd_fit_dpmgpd_sb <- function(x, ...) {
  cat("DPmixGPD fit (DPMGPD-SB)\n")
...
#' @export
summary.mixgpd_fit_dpmgpd_sb <- function(object, prob = c(0.025, 0.975), ...) {
  d <- object$draws
  out <- list()
...
#' @export
predict.mixgpd_fit_dpmgpd_sb <- function(object, newdata = NULL,
                                        type = c("sample", "mean"),
                                        n = 1L, ...) {
  type <- match.arg(type)
  spec <- object$spec
