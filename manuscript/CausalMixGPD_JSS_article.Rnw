...
<<app_cluster_overlay, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE, fig.width=8, fig.height=5.5, out.width='0.85\\linewidth', fig.pos='!htbp', fig.cap='Representative training-cluster summary for the Boston \\code{medv} analysis, returned by the \\code{plot()} method applied to the labels object from \\code{predict(fit\\_clust, type = "label")}. Training observations are shown on the response scale and coloured by their assigned cluster in the representative partition.'>>=
