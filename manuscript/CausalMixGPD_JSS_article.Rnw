...
Table~\ref{tab:app_cluster_test_profiles} gives the median response and covariate
...
Table~\ref{tab:posterior-code} reports wall-clock MCMC run times for each
...
side from the spliced causal model: the estimated QTE profile across the
...
The QTEs remain negative at the specified quantile levels, indicating that the treatment distribution lies below the control at those points of the marginal distribution. The estimated marginal quantiles for both arms show that the deviations are not reducible to a location shift but reflect heterogeneity across the outcome distribution.
