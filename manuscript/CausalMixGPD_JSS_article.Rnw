...
library("patchwork")
knitr::render_sweave()
knitr::opts_knit$set(root.dir = manuscript_dir)
cmgpd_fig_width <- 7
cmgpd_fig_height <- 4
...
fit <- dpmgpd(formula = y ~ x1 + x2 + x3, data = dat, backend = "crp", kernel = "gamma", 
    mcmc = mcmc_fixed)
@
...
fit <- dpmix(formula = y ~ x1 + x2 + x3, data = dat, backend = "crp", kernel = "gamma", 
    mcmc = mcmc_fixed)
@
...
x_new <- as.data.frame(lapply(dat[, c("x1", "x2", "x3")], quantile, probs = c(0.25, 
    0.5, 0.75), na.rm = TRUE))
...
fit_cluster <- dpmix.cluster(y ~ x1 + x2 + x3, data = dat_cl[1:90, ], kernel = "laplace", 
...
causal_fit <- dpmgpd.causal(formula = y ~ x1 + x2 + x3, data = causal_df, treat = "A", 
    backend = "crp", kernel = "laplace", components = 5, PS = "logit", ps_scale = "logit", 
    ps_summary = "mean", mcmc_outcome = mcmc_fixed, mcmc_ps = mcmc_fixed)
@
...
causal_xgrid <- expand.grid(lapply(causal_df[c("x1", "x2", "x3")], quantile, probs = c(0.25, 
    0.5, 0.75)))
@
...
fit_clust <- dpmix.cluster(formula = medv ~ lstat + rm + nox, data = train_dat, 
    kernel = "normal", type = "weights", components = 10, mcmc = mcmc_fixed)
@
...
cqte_fit <- cqte(fit, probs = c(0.25, 0.5, 0.75, 0.9, 0.95), newdata = xnew, interval = "credible")
