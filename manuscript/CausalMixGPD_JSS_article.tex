...
\begin{Schunk}
\begin{Sinput}
R> causal_fit <- dpmgpd.causal(formula = y ~ x1 + x2 + x3, data = causal_df, treat = "A",
...
\begin{Schunk}
\begin{Sinput}
R> qtt <- qtt(causal_fit, probs = c(0.50, 0.90, 0.95), interval = "credible")
R> summary(qtt)
...
R> 
R> n <- nrow(dat)
R> idx_train <- sample(seq_len(n), size = floor(0.80 * n), replace = FALSE)
...
{\centering \includegraphics[width=0.84\linewidth]{figure/app_cluster_response_panels-1} 

}

\caption{Full-sample distribution of median house values (\texttt{medv}, in thousands of US dollars) for the $506$ census tracts in the \texttt{MASS::Boston} data set. Left panel: boxplot with upper outliers highlighted. Right panel: density-scaled histogram on the same variable.}\label{fig:app_cluster_response_panels}
\end{figure}
\end{Schunk}
...
\caption[Representative training-cluster summary for the Boston \texttt{medv} analysis, returned by the \texttt{plot()} method applied to the labels object from \texttt{predict(fit\_clust, type = "label")}]{Representative training-cluster summary for the Boston \texttt{medv} analysis, returned by the \texttt{plot()} method applied to the labels object from \texttt{predict(fit\_clust, type = "label")}. Training observations are shown on the response scale and coloured by their assigned cluster in the representative partition.}\label{fig:app_cluster_overlay}
\end{figure}
\end{Schunk}
...
\caption{Bar plot of predicted cluster assignments for the held-out Boston test sample (the $20\%$ of \texttt{medv} observations excluded from training), returned by \texttt{plot(predict(fit\_clust, newdata = test\_dat, type = "label"), type = "sizes")}. Each bar corresponds to one representative training cluster from Figure~\ref{fig:app_cluster_overlay}; bar heights give the number of test tracts assigned to that cluster.}\label{fig:app_cluster_test_sizes}
\end{figure}
\end{Schunk}
...
1 430.8 18.38 239.29 173.09         FALSE
\end{Soutput}
\end{Schunk}
...
+  )
R> summary(cate_fit)
R> # plot(cate_fit, type = "effect") # Displays mean-based CATE plot by profile
\end{Sinput}
\end{Schunk}
...
+    interval = "credible"
+  )
R> # cqte_fit$fit_df # Displays the full CQTE posterior summary data frame
\end{Sinput}
\end{Schunk}
