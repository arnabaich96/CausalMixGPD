...
%% ---- Compact layout tuning for plain article style ----
...
cmgpd_pkg_version <- as.character(read.dcf(file.path(pkg_root, "DESCRIPTION"),
...
selection, and post-analysis \citep{PReMiuM_pkg}.In addition to \proglang{R}, \pkg{pyrichlet} implements Bayesian nonparametric

density estimation and clustering via Gaussian mixtures with random partition priors like

Dirichlet, Pitman-Yor processes, etc. \citep{pyrichlet_pkg},
...
likelihood on clustering, while better truncation approximations help to implement them efficiently in standard MCMC schemes

\citep{Wade2014EDP,BurnsDaniels2023EDPM}.

These and other similar software implementations and techniques are highly relevant to
...
that we have added: the DPM body model,

the GPD tail extrapolation method, the predictor-dependent clustering,

the posterior predictions, and the treatment effect comparison using \proglang{R} generics.
...
\Sexpr{cmgpd_pkg_version} has been used to generate the results presented
in this manuscript \citep{CausalMixGPD_pkg}.
Contributions from a methodology viewpoint have been documented in
\citet{Aich2026ExtremeCQTE}; the name \pkg{CausalMixGPD} is an alias for
...
<<app_cluster_fit-code, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE, cache=FALSE>>=
.clust_rds <- file.path(manuscript_dir, "saved_fits", "fit_clust.rds")
if (file.exists(.clust_rds)) {
...
arm when timing information is available.
...
summary(cqte_fit)$effect_table  # full CQTE summary table
@
...
Version \Sexpr{cmgpd_pkg_version} of the \pkg{CausalMixGPD} R package is the version used
