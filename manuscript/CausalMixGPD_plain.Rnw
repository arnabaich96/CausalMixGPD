...
% Concordance (.Rnw line numbers vs woven .tex) is required for PDF<->source sync:
%   - Sweave: \SweaveOpts{concordance=TRUE} in the preamble; or
%   - knitr: knitr::opts_knit$set(concordance = TRUE) in a setup chunk (this file).
% pdflatex SyncTeX alone maps the PDF to the intermediate .tex. To map the PDF to
% this .Rnw in the viewer, install patchDVI and either build with
%   patchDVI::knitPDF("CausalMixGPD_JSS_article.Rnw")
% (from directory manuscript/) or, after knit2pdf, run
%   patchDVI::patchSynctex("CausalMixGPD_JSS_article")
\makeatletter
\def\input@path{{./}{assets/}{manuscript/}{manuscript/assets/}}
\makeatother
\documentclass[11pt]{article}
% pdfLaTeX SyncTeX (forward/inverse search with the PDF); keep this in the .Rnw preamble.
\synctex=1
...
%% ---- JSS compatibility macros for plain article rendering ----
\newcommand{\pkg}[1]{\textsf{#1}}
\newcommand{\proglang}[1]{\texttt{#1}}
\newcommand{\code}[1]{\texttt{#1}}
\newcommand{\cmgpdAbstract}{}
\newcommand{\cmgpdKeywords}{}
\newcommand{\cmgpdAddress}{}
\newcommand{\Plainauthor}[1]{}
\newcommand{\Plaintitle}[1]{}
\newcommand{\Shorttitle}[1]{}
\newcommand{\Address}[1]{\renewcommand{\cmgpdAddress}{#1}}
\newcommand{\Abstract}[1]{\renewcommand{\cmgpdAbstract}{#1}}
\newcommand{\Keywords}[1]{\renewcommand{\cmgpdKeywords}{#1}}
...
\addtolength{\textheight}{1.2\baselineskip}
...
\setlength{\preXLskip}{1.1\baselineskip plus 0.2ex minus 0ex}
\setlength{\preMskip}{0.55\baselineskip plus 0.1ex minus 0ex}
\setlength{\preSskip}{0.45\baselineskip plus 0.1ex minus 0ex}
\setlength{\postMskip}{0.2\baselineskip plus 0ex minus 0.1ex}
\setlength{\postSskip}{0.1\baselineskip plus 0ex minus 0.1ex}
...
\RecustomVerbatimEnvironment{Sinput}{Verbatim}{fontsize=\small, fontshape=sl}
\RecustomVerbatimEnvironment{Soutput}{Verbatim}{fontsize=\small}
\RecustomVerbatimEnvironment{Scode}{Verbatim}{fontsize=\small, fontshape=sl}
\renewenvironment{Schunk}{\vspace{0pt}}{\vspace{-0.1\baselineskip}}
\makeatletter
\patchcmd{\@maketitle}{\vskip 1in}{\vskip 0.55in}{}{}
\patchcmd{\@maketitle}{\vskip 0.2in plus 1fil minus 0.1in}{\vskip 0.12in plus 0.5fil minus 0.05in}{}{}
\patchcmd{\@maketitle}{\vskip 0.3in minus 0.1in}{\vskip 0.18in minus 0.05in}{}{}
\patchcmd{\@maketitle}{\vskip 0.1in minus 0.05in}{\vskip 0.06in minus 0.03in}{}{}
\patchcmd{\@maketitle}{\vskip 0.2in minus 0.1in}{\vskip 0.1in minus 0.05in}{}{}
\makeatother

%% ---- Metadata ----
\author{Arnab Aich\\Florida State University \and Indrabati Bhattacharya\\Florida State University}
\date{}
...
densities, quantiles, means, survival summaries, and causal contrasts,
...
\cmgpdAbstract
\end{abstract}
\noindent\textbf{Keywords:} \cmgpdKeywords
...
manuscript_dir <- local({
    input_dir <- tryCatch({
        res <- knitr::current_input(dir = TRUE)
        if (is.null(res)) "" else res
    }, error = function(e) "")
    candidates <- unique(c(if (nzchar(input_dir)) input_dir, "manuscript", file.path("vignettes",
        "manuscript"), "."))
    ...
    stop("Could not locate the manuscript directory from the current working directory.",
        call. = FALSE)
})
pkg_root <- local({
...
if (requireNamespace("pkgload", quietly = TRUE)) {
    ...
    stop("Install 'CausalMixGPD' or 'pkgload' before knitting the manuscript.",
        call. = FALSE)
}
...
library("patchwork")
knitr::render_sweave()
knitr::opts_knit$set(root.dir = manuscript_dir, concordance = TRUE)
cmgpd_fig_width <- 7
cmgpd_fig_height <- 4
cmgpd_cache_path <- paste0(normalizePath(file.path(manuscript_dir, "knitr-cache"),
    winslash = "/", mustWork = FALSE), "/")
if (!dir.exists(cmgpd_cache_path)) {
    dir.create(cmgpd_cache_path, recursive = TRUE, showWarnings = FALSE)
}
bibinputs_existing <- Sys.getenv("BIBINPUTS")
Sys.setenv(BIBINPUTS = paste(c(manuscript_dir, bibinputs_existing[nzchar(bibinputs_existing)]),
    collapse = .Platform$path.sep))
bstinputs_existing <- Sys.getenv("BSTINPUTS")
Sys.setenv(BSTINPUTS = paste(c(manuscript_dir, bstinputs_existing[nzchar(bstinputs_existing)]),
    collapse = .Platform$path.sep))
knitr::opts_chunk$set(fig.width = cmgpd_fig_width, fig.height = cmgpd_fig_height,
    fig.align = "center", fig.pos = "!htbp", tidy = FALSE, echo = TRUE, eval = FALSE,
    message = FALSE, warning = FALSE, error = FALSE, tidy.opts = list(width.cutoff = 72),
    autodep = TRUE, cache = TRUE, cache.path = cmgpd_cache_path)
options(prompt = "R> ", continue = "+  ", width = 65, useFancyQuotes = FALSE, knitr.graphics.rel_path = FALSE)
cmgpd_seed <- 2026
cmgpd_pal <- c(navy = "#183A5A", teal = "#2B6F77", copper = "#B16A3A", gold = "#C59B4D",
    sage = "#5F7F68", rose = "#A64B5A", slate = "#5C6773", mist = "#D7E3ED")
cmgpd_theme <- function(base_size = 14) {
    ggplot2::theme_minimal(base_size = base_size, base_family = "serif") + ggplot2::theme(plot.title = ggplot2::element_text(face = "bold",
        colour = cmgpd_pal[["navy"]]), plot.subtitle = ggplot2::element_text(colour = cmgpd_pal[["slate"]]),
        axis.title = ggplot2::element_text(colour = cmgpd_pal[["navy"]]), axis.text = ggplot2::element_text(colour = cmgpd_pal[["slate"]]),
        legend.title = ggplot2::element_text(face = "bold", colour = cmgpd_pal[["navy"]]),
        legend.text = ggplot2::element_text(colour = cmgpd_pal[["slate"]]), legend.position = "top",
        panel.grid.minor = ggplot2::element_blank(), panel.grid.major.x = ggplot2::element_blank())
}
mcmc_fixed <- list(niter = 2000, nburnin = 500, thin = 1, nchains = 1, seed = cmgpd_seed)
@
...
\citep{BCEE_pkg}.  While these tools are
...
densities directly and deriving quantiles, means, survival summaries, and
...
clustering probabilities \citep{Binder1978} and representative
...
analysis \citep{POT_pkg}.  These existing tools typically
...
\pkg{nimble} \citep{nimble_pkg}; version 0.7.0 is available from
CRAN \citep{CausalMixGPD_pkg}.  The remainder of this article is
...
u <- 2
y <- seq(u, u + 5, length.out = 500)
sigma <- 1.2
xi_vals <- c(-0.5, 0, 0.5)
xi_legend_levels <- sprintf("xi==%.2f", xi_vals)
gpd_df <- do.call(rbind, lapply(xi_vals, function(xi) {
    data.frame(y = y, density = dgpd(y, threshold = u, scale = sigma, shape = xi),
        xi = factor(sprintf("xi==%.2f", xi), levels = xi_legend_levels))
}))
gpd_df <- gpd_df[is.finite(gpd_df$density), , drop = FALSE]
gpd_density_plot <- ggplot(gpd_df, aes(x = y, y = density, color = xi)) + geom_line(linewidth = 0.9) +
    geom_vline(xintercept = u, linetype = "dashed", linewidth = 0.7, color = cmgpd_pal[["slate"]]) +
    annotate("text", x = u + 0.05, y = 0.2, label = "threshold", hjust = 0, vjust = 1.5,
        size = 4.1, color = cmgpd_pal[["slate"]]) + scale_color_manual(values = c(cmgpd_pal[["sage"]],
    cmgpd_pal[["navy"]], cmgpd_pal[["copper"]]), labels = scales::label_parse()) +
    labs(title = "Generalized Pareto tail density by shape parameter", x = "Outcome y",
        y = "Density", color = NULL) + cmgpd_theme(base_size = 14) + theme(legend.text = element_text(size = 13),
    legend.key.width = grid::unit(1.1, "cm"), legend.key.height = grid::unit(0.6,
        "cm"))
@

<<gpdDensityFigure-plot, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, fig.width=7.0, fig.height=4.0, out.width='0.80\\linewidth', fig.pos='!htbp', fig.cap='Generalized Pareto distribution (GPD) density above a fixed threshold $u=2$ with scale $\\sigma=1.2$ and three values of the shape parameter, $\\xi\\in\\{-0.5,\\,0,\\,0.5\\}$. The dashed vertical line marks the threshold $u$.'>>=
gpd_density_plot
@
...
f_single <- dgammamix(y, w = c(1), shape = c(6), scale = c(1))
w_bulk <- c(0.6, 0.4)
shape_mix <- c(12, 16)
scale_mix <- c(0.3, 0.8)
f_bulk_mix <- dgammamix(y, w = w_bulk, shape = shape_mix, scale = scale_mix)
u80 <- qgammamix(0.8, w = w_bulk, shape = shape_mix, scale = scale_mix)
tail_scale <- 2
tail_shape <- 0.25
f_mix_gpd <- dgammamixgpd(y, w = w_bulk, shape = shape_mix, scale = scale_mix,
    threshold = u80, tail_scale = tail_scale, tail_shape = tail_shape)
spliced_df <- rbind(data.frame(y = y, density = f_single, component = "Gamma"),
    data.frame(y = y, density = f_bulk_mix, component = "Gamma mix"), data.frame(y = y,
        density = f_mix_gpd, component = "Gamma mix + GPD"))
spliced_df$component <- factor(spliced_df$component, levels = c("Gamma", "Gamma mix",
    "Gamma mix + GPD"))
ylab <- 0.92 * max(spliced_df$density, na.rm = TRUE)
spliced_density_plot <- ggplot(spliced_df, aes(x = y, y = density, color = component,
    linetype = component)) + geom_line(linewidth = 0.95) + geom_vline(xintercept = u80,
    linetype = "solid", linewidth = 0.7, color = cmgpd_pal[["gold"]]) + annotate("text",
    x = u80 + 0.13, y = ylab, label = "Threshold", hjust = 0, size = 5, fontface = "bold",
    color = cmgpd_pal[["slate"]]) + scale_color_manual(values = c(Gamma = cmgpd_pal[["sage"]],
    `Gamma mix` = cmgpd_pal[["navy"]], `Gamma mix + GPD` = cmgpd_pal[["gold"]])) +
    scale_linetype_manual(values = c(Gamma = "solid", `Gamma mix` = "longdash",
        `Gamma mix + GPD` = "dotdash")) + labs(title = "Density comparison", x = "Outcome y",
    y = "Density", color = NULL, linetype = NULL) + cmgpd_theme(base_size = 14) +
    theme(legend.box = "vertical", legend.spacing.y = grid::unit(0.05, "cm"))
@

<<splicedDensityFigure-plot, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, fig.width=7.0, fig.height=4.0, out.width='0.80\\linewidth', fig.pos='!htbp', fig.cap='Three density curves on the same axes: a single-component gamma density (solid), a two-component gamma mixture density (long-dash), and a spliced gamma-mixture-plus-GPD density (dot-dash) with GPD tail scale $2.0$ and shape $0.25$. The vertical line marks the splice threshold, set at the 80th percentile of the two-component gamma bulk mixture.'>>=
...
f0_vals <- dgammamixgpd(y_seq, w = w0, shape = shape0, scale = scale0, threshold = thr0,
    tail_scale = 1.4, tail_shape = 0.18)
w1 <- c(0.45, 0.55)
shape1 <- c(11, 20)
scale1 <- c(0.52, 0.72)
thr1 <- qgammamix(0.85, w = w1, shape = shape1, scale = scale1)
f1_vals <- dgammamixgpd(y_seq, w = w1, shape = shape1, scale = scale1, threshold = thr1,
    tail_scale = 2, tail_shape = 0.28)
dy <- diff(y_seq[1:2])
...
arm_df <- rbind(data.frame(y = y_seq, density = f0_vals, arm = "Control (a = 0)"),
...
causal_estimand_plot <- ggplot(arm_df, aes(x = y, y = density, color = arm, linetype = arm)) +
    geom_line(linewidth = 0.95) + geom_vline(xintercept = q0_90, linetype = "dotted",
    linewidth = 0.7, color = cmgpd_pal[["teal"]]) + geom_vline(xintercept = q1_90,
    linetype = "dotted", linewidth = 0.7, color = cmgpd_pal[["copper"]]) + annotate("segment",
    x = q0_90, xend = q1_90, y = ymax_annot, yend = ymax_annot, arrow = grid::arrow(ends = "both",
        length = grid::unit(0.18, "cm"), type = "closed"), color = cmgpd_pal[["slate"]],
    linewidth = 0.7) + annotate("text", x = (q0_90 + q1_90)/2, y = ymax_annot,
    label = sprintf("CQTE(0.9~'|'~x) == %.1f", q1_90 - q0_90), parse = TRUE, vjust = -0.5,
    size = 3.8, color = cmgpd_pal[["slate"]]) + scale_color_manual(values = c(cmgpd_pal[["teal"]],
    cmgpd_pal[["copper"]])) + scale_linetype_manual(values = c("solid", "longdash")) +
    labs(title = "Treatment-specific conditional densities and QTE", x = "Outcome y",
        y = "Density", color = NULL, linetype = NULL) + cmgpd_theme(base_size = 14)
@

<<causalEstimandFigure-plot, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, fig.width=7.0, fig.height=4.0, out.width='0.80\\linewidth', fig.pos='!htbp', fig.cap='Two arm-specific conditional outcome densities $f_a(y\\mid x)$, $a\\in\\{0,1\\}$, at a representative covariate profile $x$ under a spliced gamma-mixture-plus-GPD model: control arm ($a=0$, solid) and treated arm ($a=1$, long-dash). Dotted vertical lines mark the 90th conditional quantiles $Q_0(0.9\\mid x)$ and $Q_1(0.9\\mid x)$, and the double-headed arrow connects them, labelled with the conditional quantile treatment effect $\\mathrm{CQTE}(0.9\\mid x)=Q_1(0.9\\mid x)-Q_0(0.9\\mid x)$.'>>=
...
fit <- dpmgpd(formula = y ~ x1 + x2 + x3, data = dat, backend = "crp",
...
fit <- dpmix(formula = y ~ x1 + x2 + x3, data = dat, backend = "crp",
                kernel = "gamma",  mcmc = mcmc_fixed)
@
...
plot(fit, family = "auto")  # automatic selection of diagnostic plots
@
...
predict(fit, newdata = x_new, type = "quantile", index = c(0.5, 0.99),
                           interval = "credible")
@
...
fit_cluster <- dpmix.cluster(y ~ x1 + x2 + x3, data = dat_cl[1:90, ],
...
predict(fit_cluster, newdata = dat_cl[91:100, ], type = "label")
@
...
qtt_fit <- qtt(causal_fit, probs = c(0.5, 0.9, 0.95), interval = "credible")
summary(qtt_fit)
...
storage.mode(causal_xgrid) <- "double"
causal_xgrid <- causal_xgrid[, colnames(causal_fit$bundle$data$X)
                             , drop = FALSE]
@

<<overview-causal-predict-contrast, echo=TRUE, eval=FALSE>>=
predict(causal_fit, newdata = causal_xgrid,  type = "survival",
...
<<app_cluster_response_panels, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, fig.width=10, fig.height=3.8, out.width='0.86\\linewidth', fig.pos='!htbp', fig.cap='Full-sample distribution of median house values (\\code{medv}, in thousands of US dollars) for the $506$ census tracts in the \\code{MASS::Boston} data set. Left panel: boxplot with upper outliers highlighted. Right panel: density-scaled histogram on the same variable.'>>=
app_cluster_plot_df <- data.frame(medv = dat$medv, sample = "All tracts")
app_cluster_box_plot <- ggplot(app_cluster_plot_df, aes(x = sample, y = medv)) +
    geom_boxplot(width = 0.45, fill = cmgpd_pal[["mist"]], colour = cmgpd_pal[["navy"]],
        outlier.colour = cmgpd_pal[["rose"]], outlier.alpha = 0.85) + labs(title = "House prices: boxplot",
    x = NULL, y = "medv") + cmgpd_theme(base_size = 12) + theme(legend.position = "none")
app_cluster_hist_plot <- ggplot(app_cluster_plot_df, aes(x = medv, y = after_stat(density))) +
    geom_histogram(bins = 30, fill = cmgpd_pal[["mist"]], colour = "white", linewidth = 0.25) +
    labs(title = "House prices: histogram", x = "medv", y = "Density") + cmgpd_theme(base_size = 12)
...
<<app_cluster_overlay, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE, fig.width=8, fig.height=4.5, out.width='0.84\\linewidth', fig.pos='!htbp', fig.cap='Representative training-cluster summary for the Boston \\code{medv} analysis, returned by the \\code{plot()} method applied to the labels object from \\code{predict(fit\\_clust, type = "label")}. Training observations are shown on the response scale and coloured by their assigned cluster in the representative partition.'>>=
...
\code{summary(predict(fit\_clust, newdata = test\_dat, type = "label"))\$cluster\_profiles}.

<<app_cluster_test_profiles, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE>>=
summary(z_test)$cluster_profiles
@
<<app_cluster_test_sizes, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE, fig.width=7, fig.height=3.5, out.width='0.80\\linewidth', fig.pos='!htbp', fig.cap='Bar plot of predicted cluster assignments for the held-out Boston test sample (the $20\\%$ of \\code{medv} observations excluded from training), returned by \\code{plot(predict(fit\\_clust, newdata = test\\_dat, type = "label"), type = "sizes")}. Each bar corresponds to one representative training cluster from Figure~\\ref{fig:app_cluster_overlay}; bar heights give the number of test tracts assigned to that cluster.'>>=
...
app_lalonde_x_formula <- ~age_z + educ_z + race + married + nodegree + re74_z +
    re75_z
app_lalonde_y <- (app_lalonde_covars$re78 + 0.5)/1000
app_lalonde_A <- as.integer(app_lalonde_covars$treat)
app_lalonde_taus <- c(0.25, 0.5, 0.75, 0.9, 0.95)
...
<<app_lalonde_response_panels, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, fig.width=10, fig.height=4.8, out.width='0.88\\linewidth', fig.pos='!htbp', fig.cap='Full-sample distribution of 1978 annual earnings (\\code{re78}, in US dollars) from the \\code{MatchIt::lalonde} data set, stratified by treatment arm. Left panel: arm-specific boxplots with upper outliers highlighted. Right panel: overlaid density-scaled histograms for the control and treated arms.'>>=
app_lalonde_plot_df <- data.frame(re78 = app_lalonde_covars$re78, treat_label = factor(app_lalonde_covars$treat,
    levels = c(0, 1), labels = c("Control", "Treated")))
app_lalonde_box_plot <- ggplot(app_lalonde_plot_df, aes(x = treat_label, y = re78,
    fill = treat_label)) + geom_boxplot(width = 0.55, colour = cmgpd_pal[["navy"]],
    outlier.colour = cmgpd_pal[["rose"]], outlier.alpha = 0.85) + scale_fill_manual(values = c(cmgpd_pal[["mist"]],
    cmgpd_pal[["copper"]])) + labs(title = "Boxplot of 1978 earnings", x = NULL,
    y = "re78 (USD)", fill = NULL) + cmgpd_theme(base_size = 12) + theme(legend.position = "bottom")
app_lalonde_hist_plot <- ggplot(app_lalonde_plot_df, aes(x = re78, y = after_stat(density),
    fill = treat_label, colour = treat_label)) + geom_histogram(bins = 35, position = "identity",
    linewidth = 0.2, alpha = 0.45) + scale_colour_manual(values = c(cmgpd_pal[["teal"]],
    cmgpd_pal[["rose"]])) + scale_fill_manual(values = c(cmgpd_pal[["mist"]], cmgpd_pal[["copper"]])) +
    labs(title = "Histogram of 1978 earnings", x = "re78 (USD)", y = "Density") +
    cmgpd_theme(base_size = 12) + theme(legend.position = "none")
app_lalonde_box_plot + app_lalonde_hist_plot + plot_layout(ncol = 2, guides = "collect") &
...
fit <- dpmgpd.causal(y = app_lalonde_y, X = app_lalonde_X,
...
The printed output below reports wall-clock MCMC run times for each
arm returned by \code{fit\$timing}.
...
qte_fit <- qte(fit, probs = c(0.25, 0.5, 0.75), interval = "credible")
@
...
numeric_quantile <- function(x, prob = 0.5) {
    as.numeric(stats::quantile(x, probs = prob, na.rm = TRUE, names = FALSE))
}
profile_raw <- data.frame(profile = c("Profile 1", "Profile 2", "Profile 3", "Profile 4"),
    age = c(numeric_quantile(app_lalonde_covars$age, 0.25), numeric_quantile(app_lalonde_covars$age,
        0.5), numeric_quantile(app_lalonde_covars$age, 0.75), numeric_quantile(app_lalonde_covars$age,
        0.9)), educ = c(numeric_quantile(app_lalonde_covars$educ, 0.25), numeric_quantile(app_lalonde_covars$educ,
        0.5), numeric_quantile(app_lalonde_covars$educ, 0.75), numeric_quantile(app_lalonde_covars$educ,
        0.9)), race = factor(c("black", "hispan", "white", "black"), levels = levels(app_lalonde_covars$race)),
    married = factor(c("no", "no", "yes", "yes"), levels = levels(app_lalonde_covars$married)),
    nodegree = factor(c("yes", "no", "no", "yes"), levels = levels(app_lalonde_covars$nodegree)),
    re74 = c(numeric_quantile(app_lalonde_covars$re74, 0.2), numeric_quantile(app_lalonde_covars$re74,
        0.4), numeric_quantile(app_lalonde_covars$re74, 0.6), numeric_quantile(app_lalonde_covars$re74,
        0.8)), re75 = c(numeric_quantile(app_lalonde_covars$re75, 0.2), numeric_quantile(app_lalonde_covars$re75,
        0.4), numeric_quantile(app_lalonde_covars$re75, 0.6), numeric_quantile(app_lalonde_covars$re75,
        0.8)), row.names = c("p1", "p2", "p3", "p4"), check.names = FALSE)
...
xnew <- stats::model.matrix(app_lalonde_x_formula, data = profile_model)[, -1,
    drop = FALSE]
xnew <- xnew[, colnames(app_lalonde_X), drop = FALSE]
rownames(xnew) <- profile_raw$profile
profile_display <- profile_raw[, c("profile", "age", "educ", "race", "married",
    "nodegree", "re74", "re75"), drop = FALSE]
rownames(profile_display) <- NULL
@

<<xnew-results, echo=FALSE, eval=TRUE, results='asis'>>=
profile_t <- as.data.frame(t(profile_display[, -which(names(profile_display) ==
    "profile")]))
...
kableExtra::kable_styling(kableExtra::kbl(profile_t, format = "latex", booktabs = TRUE,
    ...
    row.names = TRUE), latex_options = c("scale_down"), font_size = 8)
@

<<cate-call, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE>>=
cate_fit <- cate(fit, newdata = xnew, interval = "hpd")
summary(cate_fit)
@

Figure~\ref{fig:cate_plot} shows the  CATE summaries across
the four representative profiles.
<<cate_plot, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, fig.width=6.2, fig.height=4.1, out.width='0.76\\linewidth', fig.pos='!htbp', fig.cap='Profile-specific conditional average treatment effects for Lalonde 1978 earnings at the four representative covariate profiles (Profile~1--Profile~4) of Table~\\ref{tab:xnew-results}, returned by \\code{cate(fit, newdata = xnew, interval = "hpd")}. Points denote posterior estimates of $\\mathrm{CATE}(x)=\\mu_1(x)-\\mu_0(x)$ and vertical bars give $95\\%$ HPD intervals.'>>=
cate_plot_obj <- plot(cate_fit) + ggplot2::labs(title = "CATE by profile", x = NULL) +
    ggplot2::theme(plot.title = ggplot2::element_text(size = 10.5, face = "bold"),
        axis.title.y = ggplot2::element_text(size = 9.5), axis.text = ggplot2::element_text(size = 8.5),
...
<<cqte-call, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE>>=
cqte_fit <- cqte(fit, probs = c(0.25, 0.5, 0.75, 0.9, 0.95),
...
\appendix
\begingroup
\setlength{\parskip}{0.3ex plus 0.05ex minus 0.05ex}
\setlength{\preMskip}{0.75\baselineskip plus 0.15ex minus 0ex}
\setlength{\postSskip}{0.18\baselineskip plus 0ex minus 0.05ex}
\setlist[itemize]{topsep=4pt, itemsep=3pt, parsep=1pt, partopsep=0pt}
...
\item \textbf{Parameter specification and prior setting}: the argument \code{param\_specs} is a named list giving the parameters of the kernel or tail distribution. Each kernel/tail parameter supports three modes: \code{"fixed"}, \code{"dist"}, and \code{"link"}. With \code{"dist"}, the prior can be chosen as any distribution compatible with the parameter and supported by \pkg{nimble}, with hyperparameters specified in the default registry. With \code{"link"}, the kernel/tail parameter is related to the covariates through a linear regression using one of five link functions (\code{"identity"}, \code{"exp"}, \code{"log"}, \code{"softplus"}, or \code{"power"}); the link power is adjusted via \code{link\_power} when the last option is selected. With \code{"fixed"}, the parameter is fixed to the constant value specified in \code{param\_specs}. For causal models, \code{param\_specs} can be either a single named list or a length-two list specifying different parameter specifications for the two arms.
\end{itemize}
