...
causal_run_mcmc <- list(show_progress = FALSE, timing = TRUE, parallel_arms = TRUE, workers = max_workers)

use_fast_mcmc <- TRUE
mcmc_long <- list(
  niter = 2000, nburnin = 500, thin = 3, nchains = 3, seed = 1:3,
  timing = TRUE, parallel_chains = TRUE, workers = max_workers, z_update_every = 2
)
mcmc_fast <- list(niter = 600, nburnin = 150, thin = 2, nchains = 1, seed = 1, timing = TRUE, workers = max_workers)
...
```{r}
pred_sb <- predict(fit_sb, type = "mean", interval = "credible", workers = max_workers)
head(pred_sb$fit)
```
