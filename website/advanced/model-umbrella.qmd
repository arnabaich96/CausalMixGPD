...
fit_amoroso <- mcmc(bundle_amoroso, show_progress = FALSE)
```
...
```{r}
pred_mean <- predict(fit_amoroso, x = X, type = "mean", level = 0.90, interval = "credible")
pred_q90 <- predict(fit_amoroso, x = X, type = "quantile", index = 0.90, level = 0.90, interval = "credible")
head(pred_mean$fit)
head(pred_q90$fit)
```
