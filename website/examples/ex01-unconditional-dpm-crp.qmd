...
We'll use the `bundle` function directly which handles both specification and bundle creation.
...
fit_crp <- load_or_fit("ex01-unconditional-dpm-crp-fit_crp", mcmc(bundle_crp))
```
...
fit_components <- load_or_fit("ex01-unconditional-dpm-crp-fit_components", mcmc(bundle_components))
```
...
fit_laplace <- load_or_fit("ex01-unconditional-dpm-crp-fit_laplace", mcmc(bundle_laplace))
```
...
fit_amoroso <- load_or_fit("ex01-unconditional-dpm-crp-fit_amoroso", mcmc(bundle_amoroso))
```
