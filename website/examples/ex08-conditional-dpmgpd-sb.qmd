...
ggplot(data.frame(y = y, x1 = X[, 1]), aes(x = x1, y = y)) +
  ...
  labs(title = "Tail Outcome vs x1 (SB)", newdata ="x1", y = "y") +
  theme_minimal()
```
...
ggplot(data.frame(y = y), aes(x = y)) +
  ...
  labs(title = "Threshold for SB tail (85%)", newdata ="y", y = "Density") +
  theme_minimal()
```
...
bind_rows(df_pred_gamma, df_pred_laplace) %>%
  ...
  facet_wrap(~ model) +
  labs(title = "Conditional Density (SB + GPD)", newdata ="y", y = "Density") +
  theme_minimal() +
...
bind_rows(quant_df_gamma, quant_df_laplace) %>%
  ...
  geom_point(size = 2) +
  facet_wrap(~ model) +
  labs(title = "Tail Quantiles vs x1 (SB)", newdata ="x1", y = "Quantile", color = "Probability") +
  theme_minimal()
```
