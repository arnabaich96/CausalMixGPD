...
```{r bundle-mcmc-crp-gpd, cache=TRUE}
...
fit_gpd <- load_or_fit("ex03-unconditional-dpmgpd-crp-fit_gpd", run_mcmc_bundle_manual(bundle_gpd))
```
...
fit_bulk_only <- load_or_fit("ex03-unconditional-dpmgpd-crp-fit_bulk_only", run_mcmc_bundle_manual(bundle_bulk_only))
```
...
```{r threshold-sensitivity, cache=TRUE}
sensitivity_fits <- lapply(thresholds, function(u) {
  ...
  fit <- load_or_fit("ex03-unconditional-dpmgpd-crp-fit", run_mcmc_bundle_manual(bundle))
