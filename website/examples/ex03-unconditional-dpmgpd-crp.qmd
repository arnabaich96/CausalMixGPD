...
fit_gpd <- load_or_fit("ex03-unconditional-dpmgpd-crp-fit_gpd", mcmc(bundle_gpd))
```
...
fit_bulk_only <- load_or_fit("ex03-unconditional-dpmgpd-crp-fit_bulk_only", mcmc(bundle_bulk_only))
```
...
sensitivity_fits <- lapply(thresholds, function(u) {
  ...
  fit <- load_or_fit("ex03-unconditional-dpmgpd-crp-fit", mcmc(bundle))
