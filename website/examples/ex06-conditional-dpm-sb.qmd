...
ggplot(data.frame(y = y, x1 = X[, 1]), aes(x = x1, y = y)) +
  ...
  labs(title = "y vs X1 (SB)", newdata ="X1", y = "y") +
  theme_minimal()
```
...
ggplot(df_cond, aes(x = y, y = density, color = label)) +
  ...
  facet_wrap(~ model) +
  labs(title = "SB Conditional Predictive Densities", newdata ="y", y = "Density") +
  theme_minimal() +
...
bind_rows(quant_df_normal, quant_df_cauchy) %>%
  ...
  geom_point(size = 2) +
  facet_wrap(~ model) +
  labs(title = "SB Conditional Quantiles vs x1", newdata ="x1", y = "y", color = "Quantile") +
  theme_minimal()
```
