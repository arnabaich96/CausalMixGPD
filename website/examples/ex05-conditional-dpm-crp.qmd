...
p_cov <- ggplot(df_cov, aes(x = x1, y = y)) +
  ...
  labs(title = "y vs X1 (loess smoother)", newdata ="X1", y = "y") +
  theme_minimal()
...
ggplot(df_dens, aes(x = y, y = density, color = group)) +
  ...
  facet_wrap(~ model) +
  labs(title = "Conditional Predictive Densities", newdata ="y", y = "Density") +
  theme_minimal() +
...
bind_rows(quant_df_normal, quant_df_amoroso) %>%
  ...
  geom_point(size = 2) +
  facet_wrap(~ model) +
  labs(title = "Conditional Quantiles vs x1 (x2=0)", newdata ="x1", y = "y", color = "Quantile") +
  theme_minimal()
```
