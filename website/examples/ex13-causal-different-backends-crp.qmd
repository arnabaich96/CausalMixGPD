...
p_scatter <- ggplot(df_causal, aes(x = x1, y = y, color = A)) +
  ...
  labs(title = "Outcome vs X1", newdata ="X1", y = "y", color = "Treatment") +
  theme_minimal()
...
fit_sb_crp <- load_or_fit(
  "ex13-causal-different-backends-crp-fit_sb_crp",
  quiet_mcmc(dpmix.causal(bundle_sb_crp))
)
summary(fit_sb_crp)
params(fit_sb_crp)
```
...
```{r ate-sb-crp}
att_sb_crp <- att(fit_sb_crp, interval = "credible", nsim_mean = 100)
plot(att_sb_crp)
```
...
fit_crp_sb <- load_or_fit(
  "ex13-causal-different-backends-crp-fit_crp_sb",
  quiet_mcmc(dpmix.causal(bundle_crp_sb))
)
summary(fit_crp_sb)
params(fit_crp_sb)
```
