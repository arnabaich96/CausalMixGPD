...
```{r ate-sb-bulk}
ate_bulk <- cate(fit_sb_bulk, newdata = x_eval, interval = "credible", nsim_mean = 150)
head(ate_bulk)
safe_plot(ate_bulk)
```

```{r qte-sb-bulk}
qte_bulk <- cqte(fit_sb_bulk, probs = c(0.25, 0.5, 0.75), newdata = x_eval, interval = "credible")
head(qte_bulk)
safe_plot(qte_bulk)
```
