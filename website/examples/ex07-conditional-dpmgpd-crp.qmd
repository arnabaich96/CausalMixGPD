...
- How to model \(y \mid X\) when both the **bulk shape** and the **extreme tail** matter.
...
ggplot(data.frame(y = y), aes(x = y)) +
  ...
  labs(title = paste("Threshold at", signif(u_threshold, 3)), newdata ="y", y = "Density") +
  theme_minimal()
```
...
bind_rows(df_pred_lognormal, df_pred_normal) %>%
  ...
  facet_wrap(~ model) +
  labs(title = "Conditional Density with GPD Tail", newdata ="y", y = "Density") +
  theme_minimal() +
...
bind_rows(quant_df_lognormal, quant_df_normal) %>%
  ...
  geom_point(size = 2) +
  facet_wrap(~ model) +
  labs(title = "Tail Quantiles vs x1 (CRP)", newdata ="x1", y = "Quantile", color = "Probability") +
  theme_minimal()
```
