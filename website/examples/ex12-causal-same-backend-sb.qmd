...
```{r plot-fit-sb-bulk}
safe_plot(fit_sb_bulk, family = c("traceplot", "autocorrelation", "running"))
```
...
```{r ate-sb-bulk}
ate_bulk <- cate(fit_sb_bulk, newdata = x_eval,
                interval = "credible", nsim_mean = 100)
safe_plot(ate_bulk)
```

```{r qte-sb-bulk}
qte_bulk <- cqte(fit_sb_bulk, probs = c(0.25, 0.5, 0.75),
