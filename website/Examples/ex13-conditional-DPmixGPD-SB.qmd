---
title: "ex13. Conditional DPmixGPD with Stick-Breaking Backend"
---
...
bundle_sb_cond_gpd_gamma <- DPmixGPD::build_nimble_bundle(
  y = y,
  X = X,
  kernel = "gamma",
  backend = "sb",
...
bundle_sb_cond_gpd_laplace <- DPmixGPD::build_nimble_bundle(
  y = y,
  X = X,
  kernel = "laplace",
  backend = "sb",
...
```{r mcmc, cache=FALSE}
fit_sb_cond_gpd_gamma <- load_or_fit("ex13-conditional-DPmixGPD-SB-fit_sb_cond_gpd_gamma", run_mcmc_bundle_manual(bundle_sb_cond_gpd_gamma))
fit_sb_cond_gpd_laplace <- load_or_fit("ex13-conditional-DPmixGPD-SB-fit_sb_cond_gpd_laplace", run_mcmc_bundle_manual(bundle_sb_cond_gpd_laplace))
...
safe_plot(fitted(fit_sb_cond_gpd_gamma))
```

```{r diagnostics}
safe_plot(fit_sb_cond_gpd_gamma, family = c("traceplot", "autocorrelation", "geweke"))
safe_plot(fit_sb_cond_gpd_laplace, family = c("density", "running", "caterpillar"))
```
...
- Next: move into causal models starting with same-backend CRP (ex12).
...
- Previous: [ex12-conditional-DPmixGPD-CRP](ex12-conditional-DPmixGPD-CRP.html)
- Next: [ex14-causal-no-x-CRP](ex14-causal-no-x-CRP.html)
