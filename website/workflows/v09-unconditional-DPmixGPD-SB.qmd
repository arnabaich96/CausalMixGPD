...
This follows the same structure as the SB bulk-only vignette (`v05`): build a bundle with `DPmixGPD::build_nimble_bundle()`, run MCMC, then use the S3 `predict()` + `plot()` helpers. Compared with `v06` (CRP+GPD), here we keep the **stick-breaking** backend and use a **Gamma** bulk kernel with a lognormal threshold prior, then contrast it with a bulk-only **Laplace** fit.
...
- Previous: [v08-unconditional-DPmixGPD-CRP](v08-unconditional-DPmixGPD-CRP.html)
- Next: [v10-conditional-DPmix-CRP](v10-conditional-DPmix-CRP.html)
