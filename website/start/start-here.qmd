...
```{r start-uncond-bundle, cache=TRUE}
...
```{r start-uncond-fit, cache=TRUE}
fit_uncond <- load_or_fit("quickstart-start-here-fit_uncond", quiet_mcmc(mcmc(bundle_uncond, show_progress = FALSE)))
summary(fit_uncond)
```

```{r start-uncond-predict, cache=TRUE}
...
```{r start-cond-bundle, cache=TRUE}
bundle_cond <- bundle(
  y = yc,
  X = X,
  backend = "sb",
...
```{r start-cond-fit, cache=TRUE}
fit_cond <- load_or_fit("quickstart-start-here-fit_cond", quiet_mcmc(mcmc(bundle_cond, show_progress = FALSE)))
summary(fit_cond)
```

```{r start-cond-predict, cache=TRUE}
