...
predict(fit, newdata =X_new, type = "mean")
```
...
pred <- predict(fit, newdata =X_new, y = y_grid, type = "density", ncores = 2)
```
...
fit <- dpmix.causal(cb, mcmc = list(parallel_arms = TRUE, workers = max_workers))
```
