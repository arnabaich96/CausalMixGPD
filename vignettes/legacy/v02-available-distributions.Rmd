...
The lowercase `d*()`, `p*()`, and `q*()` functions accept vector inputs for their first argument and evaluate elementwise; `r*()` supports `n > 1` (including `n = 0`). The CamelCase versions (e.g., `dGammaMix`) are NIMBLE-compatible scalar functions for use in model code.
