...
load_or_fit <- function(tag, expr) {
  ...
  # Use match.call to capture the unevaluated expression
  mc <- match.call(expand.dots = FALSE)
  expr_sub <- mc$expr
  if (!is.null(expr_sub) && (is.language(expr_sub) || is.call(expr_sub))) {
    out <- eval(expr_sub, envir = parent.frame())
  } else {
    # Fallback: if match.call didn't capture it properly, use substitute as backup
    expr_sub <- substitute(expr)
    if (is.language(expr_sub)) {
      out <- eval(expr_sub, envir = parent.frame())
    } else {
      # Last resort: evaluate expr directly (it may already be evaluated)
      out <- expr
    }
  }
