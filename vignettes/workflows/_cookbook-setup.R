# Cookbook vignette shared setup for precomputed results.
# Sourced by all cookbook vignettes for consistent precomputed file handling.
...
.resolve_precomp_path <- function(tag) {
  ...
  candidates[[length(candidates) + 1]] <- file.path(getwd(), "vignettes", "cookbook", "articles", "cookbook-precomputed", filename)

  # Try from package root (if detectable)
  root_from_cwd <- .find_pkg_root(getwd())
  if (!is.null(root_from_cwd)) {
    candidates[[length(candidates) + 1]] <- file.path(root_from_cwd, "vignettes", "cookbook", "articles", "cookbook-precomputed", filename)
  }
  ...
  if (!is.null(vignette_dir) && nzchar(vignette_dir)) {
    candidates[[length(candidates) + 1]] <- file.path(vignette_dir, "articles", "cookbook-precomputed", filename)
    root_from_vignette <- .find_pkg_root(vignette_dir)
    if (!is.null(root_from_vignette)) {
      candidates[[length(candidates) + 1]] <- file.path(root_from_vignette, "vignettes", "cookbook", "articles", "cookbook-precomputed", filename)
    }
  }
