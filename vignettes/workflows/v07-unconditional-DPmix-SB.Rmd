---
title: "Workflow 7: Unconditional DPmix with Stick-Breaking Backend"
output: 
  rmarkdown::html_vignette:
    code_folding: show
vignette: >
  %\\VignetteIndexEntry{Workflow 7: Unconditional DPmix with Stick-Breaking Backend}
...
stick-breaking prior on mixture weights. This yields an explicit finite mixture
approximation controlled by `components`.

# Unconditional DPmix: Stick-Breaking (SB) Backend

**Purpose**: Showcase the stick-breaking backend that uses a fixed number of mixture components (`components`) and contrast it with the CRP backend from `v04`. In this vignette we fit **two** bulk kernels (**Gamma** and **Cauchy**) on the same dataset to highlight how heavier tails in the bulk can change the fit even without a GPD tail.
