...
\begin{itemize}
   \item \textbf{BNP density modeling and prediction with optional GPD tail:}
...
Section~\ref{sec:app-clustering} and Section~\ref{sec:app-causal} illustrates the package's capabilities through applications to causal inference and clustering, respectively.
...
plot(z_test, type = "summary", top_n = 5, order_by = "size")
@
