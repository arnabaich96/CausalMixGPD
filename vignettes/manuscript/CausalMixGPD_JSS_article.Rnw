...
data("nc_real200_k2", package = "CausalMixGPD")
...
<<causal-fit-model-results, echo=FALSE, eval=TRUE>>=
cfit
@
...
<<causal-ate-results, echo=FALSE, eval=TRUE>>=
...
<<causal-qte-results, echo=FALSE, eval=TRUE>>=
...
<<causal-cate-results, echo=FALSE, eval=TRUE>>=
...
<<causal-cqte-results, echo=FALSE, eval=TRUE>>=
...
<<causal-predict-quantile-results, echo=FALSE, eval=TRUE>>=
...
<<causal-predict-survival-results, echo=FALSE, eval=TRUE>>=
...
<<causal-predict-arms-results, echo=FALSE, eval=TRUE>>=
...
knitr::kable(
  ...
  caption = "Training cluster allocation (counts) \\label{tab:cluster-train-alloc}"
)
@
...
<<app_cluster_psm_plot-results, echo=FALSE, eval=TRUE>>=
cat(sprintf("PSM dimension: %d x %d\n", nrow(PSM_train$psm), ncol(PSM_train$psm)))
@
...
knitr::kable(
  as_horizontal_counts(train_cluster_counts, "train"),
  caption = "Train allocation (horizontal)"
)
@
...
knitr::kable(
  ...
  caption = "Test allocation (horizontal)"
)
@
...
<<app_cluster_old_new_barplot-results, echo=FALSE, eval=TRUE>>=
cat("Bar plot rendered above.\n")
@
...
<<app2-fit-results, echo=FALSE, eval=TRUE>>=
data.frame(
  metric = c("n", "p", "n_treated", "n_control"),
  value = c(length(y), ncol(X), sum(A == 1), sum(A == 0))
)
@
...
head(as.data.frame(cqte_fit$qte$fit), 4)
@
