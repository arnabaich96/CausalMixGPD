...
<<gpdDensityFigure-code, echo=TRUE, eval=FALSE, fig.height=4>>=
u <- 2.0
...
train_cluster_counts <- data.frame(
  cluster = names(cluster_fit_summary$cluster_sizes),
  n = as.integer(cluster_fit_summary$cluster_sizes),
  row.names = NULL,
  check.names = FALSE
)
train_cluster_order <- as.character(train_cluster_counts$cluster)
...
A response-summary view of the representative partition is shown in
Figure~\ref{fig:cluster-summary-train}, captioned ``Response distributions
across representative training clusters.'' The clusters are ordered by
decreasing allocation size from left to right.

<<app_cluster_summary_plot-code, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE, fig.width=7, fig.height=5.5, out.width='0.78\\linewidth', fig.pos='!htbp', fig.cap='Response distributions across representative training clusters. \\label{fig:cluster-summary-train}'>>=
plot(fit_clust, which = "summary")
@
...
test_cluster_counts <- data.frame(
  cluster = train_cluster_order,
  n = as.integer(table(factor(z_test$labels, levels = train_cluster_order))),
  row.names = NULL,
  check.names = FALSE
)

old_new_train_counts <- as_horizontal_counts(train_cluster_counts, "train")
old_new_test_counts  <- as_horizontal_counts(test_cluster_counts, "test")
@

<<app_cluster_old_new_train-results, echo=FALSE, eval=TRUE>>=
knitr::kable(
  old_new_train_counts,
  caption = "Training cluster sizes (comparison view)"
)
@
...
We also compare the training and test allocations graphically in
Figure~\ref{fig:cluster-old-new-summary}, captioned ``Response distributions
for training and test samples under representative labels.'' The test-sample
boxplots are overlaid on the training distributions so that the same
representative clusters can be compared directly.

<<app_cluster_old_new_summary-code, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE, fig.width=7, fig.height=5.5, out.width='0.78\\linewidth', fig.pos='!htbp', fig.cap='Response distributions for training and test samples under representative labels. \\label{fig:cluster-old-new-summary}'>>=
plot(z_test, type = "summary")
@


This analysis illustrates the intended clustering workflow: inference is
summarized through a representative training partition, ordered cluster-size
tables, and cluster-specific response summaries, and new observations are
assigned to those clusters using posterior predictive co-clustering evidence.
