...
and a clustering workflow based on posterior co-clustering
probabilities. This breadth is useful in applications where the same
dataset motivates more than one inferential target: exploratory density
estimation, subgroup discovery, prediction at extreme quantiles, and
distributional treatment-effect reporting can all be pursued without
switching to unrelated software.
...
applications. From a software perspective, the registry-based package
architecture makes it natural to add new kernels, tail components, and
developer-facing extensions. We therefore view \pkg{CausalMixGPD} not
only as an applied package for tail-aware density estimation and causal
inference, but also as a foundation for further work on Bayesian
distributional modeling in \textsf{R}.
