...
<<gpdDensityFigure-results, echo=FALSE, eval=TRUE>>=
...
<<splicedDensityFigure-results, echo=FALSE, eval=TRUE>>=
...
quantiles, and tail risks rather than only mean-scale effects. For datasets with moderate to light tail behavior,
 users can choose to fit a DPM without splicing by using appropriate settings for the tail (See Section ~\eqref{subsec:overview-nogpd}).
...
<<overview-mcmc-settings, echo=TRUE, eval=FALSE>>=
mcmc_fixed <- list(
  niter = 200,
  nburnin = 50,
  thin = 1,
...
<<causal-mcmc-settings, echo=TRUE, eval=FALSE>>=
mcmc_fixed <- list(
  niter = 200,
  nburnin = 50,
  thin = 1,
...
<<app_cluster_fit-cache, echo=FALSE, message=FALSE, warning=FALSE, results='hide', eval=TRUE, include=FALSE>>=
...
<<app_cluster_fit-results, echo=FALSE, eval=TRUE>>=
data.frame(
  sample = c("train", "new"),
...
<<app_cluster_newdata_allocation-cache, echo=FALSE, message=FALSE, warning=FALSE, results='hide', eval=TRUE, include=FALSE>>=
obj <- read_cache_rds("tables", "app_cluster_newdata_allocation.rds")
list2env(obj, envir = environment())
@

<<app_cluster_newdata_allocation-code, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE>>=
cluster_alloc_new <- allocation(fit_clust, newdata = newdata_cluster)
@

<<app_cluster_newdata_allocation-results, echo=FALSE, eval=TRUE>>=
train_cluster_counts
new_cluster_counts
cluster_certainty
@

<<app_cluster_newdata_barplot-code, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE, fig.width=7, fig.height=5>>=
...
<<app_cluster_newdata_barplot-results, echo=FALSE, eval=TRUE>>=
...
<<app2-mcmc-settings, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE>>=
mcmc_fixed <- list(
  niter = 120,
  nburnin = 40,
  thin = 1,
...
<<app2-ate-plot-results, echo=FALSE, eval=TRUE>>=
knitr::include_graphics(cache_file("figs", "app2_ate_effect.png"))
@

<<app2-qte-plot-results, echo=FALSE, eval=TRUE>>=
