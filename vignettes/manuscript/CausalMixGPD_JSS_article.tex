...
\hlkwd{plot}\hldef{(fit,} \hlkwc{family} \hldef{=} \hlsng{"traceplot"}\hldef{,}\hlkwc{params} \hldef{=} \hlsng{"alpha"}\hldef{)}  \hlcom{# DPM concentration parameter}
\hlkwd{plot}\hldef{(fit,} \hlkwc{family} \hldef{=} \hlsng{"auto"}\hldef{)}     \hlcom{#  Default set of diagnostics for all parameters}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{causal_fit} \hlkwb{<-} \hlkwd{dpmgpd.causal}\hldef{(} \hlkwc{formula} \hldef{= y} \hlopt{~} \hldef{x1} \hlopt{+} \hldef{x2} \hlopt{+} \hldef{x3,} \hlkwc{data} \hldef{= causal_df,}
                                        \hlkwc{treat} \hldef{=} \hlsng{"A"}\hldef{,} \hlkwc{backend} \hldef{=} \hlsng{"crp"}\hldef{,} \hlkwc{kernel} \hldef{=} \hlsng{"laplace"}\hldef{,}
                                            \hlkwc{components} \hldef{=} \hlnum{5}\hldef{,} \hlkwc{PS} \hldef{=} \hlsng{"logit"}\hldef{,} \hlkwc{ps_scale} \hldef{=} \hlsng{"logit"}\hldef{,}
                                            \hlkwc{ps_summary} \hldef{=} \hlsng{"mean"}\hldef{,} \hlkwc{mcmc_outcome} \hldef{= mcmc_fixed,}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{fit_clust} \hlkwb{<-} \hlkwd{dpmix.cluster}\hldef{(} \hlkwc{formula} \hldef{= medv} \hlopt{~} \hldef{lstat} \hlopt{+} \hldef{rm} \hlopt{+} \hldef{nox,} \hlkwc{data} \hldef{= train_dat,} \hlkwc{kernel} \hldef{=} \hlsng{"normal"}\hldef{,}
...
1      C2 101  22.60099  9.2755   13.03455 7.074494
2      C3   1  24.50000      NA   13.59000       NA
   rm_mean     rm_sd  nox_mean   nox_sd
1 6.321614 0.7520175 0.5644554 0.119345
2 5.926000        NA 0.5850000       NA
\end{verbatim}
\begin{alltt}
...
\hldef{covars} \hlkwb{<-} \hlkwd{within}\hldef{(lalonde, \{}
  ...
  \hldef{age_z} \hlkwb{<-} \hlkwd{as.numeric}\hldef{(}\hlkwd{scale}\hldef{(age))}
  \hldef{educ_z} \hlkwb{<-} \hlkwd{as.numeric}\hldef{(}\hlkwd{scale}\hldef{(educ))}
  \hldef{re74_z} \hlkwb{<-} \hlkwd{as.numeric}\hldef{(}\hlkwd{scale}\hldef{(re74))}
  \hldef{re75_z} \hlkwb{<-} \hlkwd{as.numeric}\hldef{(}\hlkwd{scale}\hldef{(re75))}
\hldef{\})}
\hldef{x_formula} \hlkwb{<-} \hlopt{~} \hldef{age_z} \hlopt{+} \hldef{educ_z} \hlopt{+} \hldef{race} \hlopt{+} \hldef{married} \hlopt{+} \hldef{nodegree} \hlopt{+} \hldef{re74_z} \hlopt{+} \hldef{re75_z}
\hldef{scale_center} \hlkwb{<-} \hlkwd{c}\hldef{(}
  \hlkwc{age} \hldef{=} \hlkwd{mean}\hldef{(covars}\hlopt{$}\hldef{age,} \hlkwc{na.rm} \hldef{=} \hlnum{TRUE}\hldef{),}
  \hlkwc{educ} \hldef{=} \hlkwd{mean}\hldef{(covars}\hlopt{$}\hldef{educ,} \hlkwc{na.rm} \hldef{=} \hlnum{TRUE}\hldef{),}
  \hlkwc{re74} \hldef{=} \hlkwd{mean}\hldef{(covars}\hlopt{$}\hldef{re74,} \hlkwc{na.rm} \hldef{=} \hlnum{TRUE}\hldef{),}
  \hlkwc{re75} \hldef{=} \hlkwd{mean}\hldef{(covars}\hlopt{$}\hldef{re75,} \hlkwc{na.rm} \hldef{=} \hlnum{TRUE}\hldef{)}
\hldef{)}
\hldef{scale_scale} \hlkwb{<-} \hlkwd{c}\hldef{(}
  \hlkwc{age} \hldef{= stats}\hlopt{::}\hlkwd{sd}\hldef{(covars}\hlopt{$}\hldef{age,} \hlkwc{na.rm} \hldef{=} \hlnum{TRUE}\hldef{),}
  \hlkwc{educ} \hldef{= stats}\hlopt{::}\hlkwd{sd}\hldef{(covars}\hlopt{$}\hldef{educ,} \hlkwc{na.rm} \hldef{=} \hlnum{TRUE}\hldef{),}
  \hlkwc{re74} \hldef{= stats}\hlopt{::}\hlkwd{sd}\hldef{(covars}\hlopt{$}\hldef{re74,} \hlkwc{na.rm} \hldef{=} \hlnum{TRUE}\hldef{),}
  \hlkwc{re75} \hldef{= stats}\hlopt{::}\hlkwd{sd}\hldef{(covars}\hlopt{$}\hldef{re75,} \hlkwc{na.rm} \hldef{=} \hlnum{TRUE}\hldef{)}
\hldef{)}
\hldef{y} \hlkwb{<-} \hldef{(covars}\hlopt{$}\hldef{re78} \hlopt{/} \hlnum{1000}\hldef{)} \hlopt{+} \hlnum{0.1}  \hlcom{# Rescale to thousands and shift away from zero}
...
\hldef{X} \hlkwb{<-} \hldef{stats}\hlopt{::}\hlkwd{model.matrix}\hldef{(x_formula,} \hlkwc{data} \hldef{= covars)[,} \hlopt{-}\hlnum{1}\hldef{,} \hlkwc{drop} \hldef{=} \hlnum{FALSE}\hldef{]}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{fit} \hlkwb{<-} \hlkwd{dpmgpd.causal}\hldef{(}\hlkwc{y} \hldef{= y,} \hlkwc{X} \hldef{= X,} \hlkwc{treat} \hldef{= A,} \hlkwc{backend} \hldef{=} \hlsng{"sb"}\hldef{,} \hlkwc{kernel} \hldef{=} \hlsng{"gamma"}\hldef{,} \hlkwc{components} \hldef{=} \hlnum{10}\hldef{,}
                     \hlkwc{mcmc} \hldef{= mcmc_fixed,} \hlkwc{PS} \hldef{=} \hlsng{"logit"}\hldef{,} \hlkwc{parallel_arms} \hldef{=} \hlnum{FALSE}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{qte_fit} \hlkwb{<-} \hlkwd{qte}\hldef{(fit,} \hlkwc{probs} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{0.25}\hldef{,} \hlnum{0.50}\hldef{,} \hlnum{0.75}\hldef{,} \hlnum{0.9}\hldef{),}\hlkwc{interval} \hldef{=} \hlsng{"credible"}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{cate_fit} \hlkwb{<-} \hlkwd{cate}\hldef{(}
  ...
  \hlkwc{interval} \hldef{=} \hlsng{"hpd"}\hldef{,}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{cqte_fit} \hlkwb{<-} \hlkwd{cqte}\hldef{(}
  \hldef{fit,}
  \hlkwc{probs} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{0.25}\hldef{,} \hlnum{0.50}\hldef{,} \hlnum{0.75}\hldef{,} \hlnum{0.90}\hldef{,} \hlnum{0.95}\hldef{),}
  \hlkwc{newdata} \hldef{= xnew,}
...
\midrule
Profile 1 & 1 & 0.25 & 0.3019511 & -0.4815649 & 0.9769366\\
Profile 1 & 1 & 0.50 & 4.4856573 & -0.3702209 & 8.0188011\\
Profile 1 & 1 & 0.75 & 1.4674447 & -1.6761926 & 6.8176842\\
Profile 1 & 1 & 0.90 & 2.3396730 & -0.8661574 & 6.6256168\\
Profile 1 & 1 & 0.95 & 4.6852617 & -0.1550252 & 11.6062524\\
\addlinespace
Profile 2 & 2 & 0.25 & 1.4960550 & -0.6654031 & 7.4404329\\
Profile 2 & 2 & 0.50 & 1.7118259 & -2.9399981 & 7.7434604\\
Profile 2 & 2 & 0.75 & 1.5031277 & -4.9068030 & 7.9431551\\
Profile 2 & 2 & 0.90 & 0.1947539 & -9.2017402 & 8.5692921\\
Profile 2 & 2 & 0.95 & 0.7119869 & -9.7074149 & 11.3241264\\
\addlinespace
Profile 3 & 3 & 0.25 & 3.5635365 & 0.0362661 & 7.8824349\\
Profile 3 & 3 & 0.50 & 3.7854097 & -0.5221555 & 8.2638211\\
Profile 3 & 3 & 0.75 & 3.7443716 & -1.1726945 & 8.1122052\\
Profile 3 & 3 & 0.90 & 2.8126051 & -1.8744535 & 9.0822317\\
Profile 3 & 3 & 0.95 & 3.1654931 & -2.8369252 & 13.4850639\\
\addlinespace
Profile 4 & 4 & 0.25 & 2.7002222 & -0.0399994 & 5.4073302\\
Profile 4 & 4 & 0.50 & 3.4795806 & -3.6635255 & 9.1849038\\
Profile 4 & 4 & 0.75 & 4.0569057 & -6.2828952 & 22.7138961\\
Profile 4 & 4 & 0.90 & 11.0033083 & -10.3943889 & 51.5395831\\
Profile 4 & 4 & 0.95 & 19.8329990 & -12.3811331 & 83.1056390\\
\bottomrule
\end{tabular}}
\end{table}
