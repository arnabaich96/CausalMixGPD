...
\begin{alltt}
\hldef{u} \hlkwb{<-} \hlnum{2.0}
\hldef{y} \hlkwb{<-} \hlkwd{seq}\hldef{(u, u} \hlopt{+} \hlnum{8}\hldef{,} \hlkwc{length.out} \hldef{=} \hlnum{500}\hldef{)}
\hldef{sigma} \hlkwb{<-} \hlnum{1.2}
\hldef{xi_vals} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlopt{-}\hlnum{0.25}\hldef{,} \hlnum{0.0}\hldef{,} \hlnum{0.25}\hldef{)}

\hldef{gpd_df} \hlkwb{<-} \hlkwd{do.call}\hldef{(}
  \hldef{rbind,}
  \hlkwd{lapply}\hldef{(xi_vals,} \hlkwa{function}\hldef{(}\hlkwc{xi}\hldef{) \{}
    \hlkwd{data.frame}\hldef{(}
      \hlkwc{y} \hldef{= y,}
      \hlkwc{density} \hldef{=} \hlkwd{dgpd}\hldef{(y,} \hlkwc{threshold} \hldef{= u,} \hlkwc{scale} \hldef{= sigma,} \hlkwc{shape} \hldef{= xi),}
      \hlkwc{xi} \hldef{=} \hlkwd{factor}\hldef{(}\hlkwd{sprintf}\hldef{(}\hlsng{"xi = %.2f"}\hldef{, xi),} \hlkwc{levels} \hldef{=} \hlkwd{sprintf}\hldef{(}\hlsng{"xi = %.2f"}\hldef{, xi_vals))}
    \hldef{)}
  \hldef{\})}
\hldef{)}
\hldef{gpd_df} \hlkwb{<-} \hldef{gpd_df[}\hlkwd{is.finite}\hldef{(gpd_df}\hlopt{$}\hldef{density), ,} \hlkwc{drop} \hldef{=} \hlnum{FALSE}\hldef{]}

\hlkwd{ggplot}\hldef{(gpd_df,} \hlkwd{aes}\hldef{(}\hlkwc{x} \hldef{= y,} \hlkwc{y} \hldef{= density,} \hlkwc{color} \hldef{= xi))} \hlopt{+}
  \hlkwd{geom_line}\hldef{(}\hlkwc{linewidth} \hldef{=} \hlnum{0.9}\hldef{)} \hlopt{+}
  \hlkwd{geom_vline}\hldef{(}
    \hlkwc{xintercept} \hldef{= u,}
    \hlkwc{linetype} \hldef{=} \hlsng{"dashed"}\hldef{,}
    \hlkwc{linewidth} \hldef{=} \hlnum{0.7}\hldef{,}
    \hlkwc{color} \hldef{=} \hlsng{"gray35"}
  \hldef{)} \hlopt{+}
  \hlkwd{annotate}\hldef{(}
    \hlsng{"text"}\hldef{,}
    \hlkwc{x} \hldef{= u} \hlopt{+} \hlnum{0.05}\hldef{,}
    \hlkwc{y} \hldef{=} \hlkwd{max}\hldef{(gpd_df}\hlopt{$}\hldef{density)} \hlopt{*} \hlnum{0.95}\hldef{,}
    \hlkwc{label} \hldef{=} \hlsng{"threshold u"}\hldef{,}
    \hlkwc{hjust} \hldef{=} \hlnum{0}
  \hldef{)} \hlopt{+}
  \hlkwd{scale_color_manual}\hldef{(}\hlkwc{values} \hldef{=} \hlkwd{c}\hldef{(}\hlsng{"#1b9e77"}\hldef{,} \hlsng{"#d95f02"}\hldef{,} \hlsng{"#7570b3"}\hldef{))} \hlopt{+}
  \hlkwd{labs}\hldef{(}
    \hlkwc{title} \hldef{=} \hlsng{"GPD Tail Density Above Threshold"}\hldef{,}
    \hlkwc{x} \hldef{=} \hlsng{"Outcome y"}\hldef{,}
    \hlkwc{y} \hldef{=} \hlsng{"Density"}\hldef{,}
    \hlkwc{color} \hldef{=} \hlkwa{NULL}
  \hldef{)} \hlopt{+}
...
\caption{\label{tab:app_cluster_train_sizes-results}Training cluster sizes (counts) \label{tab:cluster-train-alloc}}
\centering
\begin{tabular}[t]{l|r|r|r|r|r|r|r|r|r|r|r}
\hline
sample & C3 & C5 & C2 & C1 & C9 & C4 & C6 & C7 & C11 & C8 & C10\\
\hline
train & 163 & 113 & 56 & 28 & 6 & 4 & 3 & 2 & 2 & 1 & 1\\
\hline
\end{tabular}
\end{table}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{plot}\hldef{(fit_clust,} \hlkwc{which} \hldef{=} \hlsng{"summary"}\hldef{)}
\end{alltt}
\end{kframe}\begin{figure}[!htbp]

{\centering \includegraphics[width=0.78\linewidth]{figure/app_cluster_summary_plot-code-1} 

}

\caption{Response distributions across representative training clusters. \label{fig:cluster-summary-train}}\label{fig:app_cluster_summary_plot-code}
\end{figure}
...
\caption{\label{tab:app_cluster_old_new_train-results}Training cluster sizes (comparison view)}
\centering
\begin{tabular}[t]{l|r|r|r|r|r|r|r|r|r|r|r}
\hline
sample & C3 & C5 & C2 & C1 & C9 & C4 & C6 & C7 & C11 & C8 & C10\\
...
sample & C3 & C5 & C2 & C1 & C9 & C4 & C6 & C7 & C11 & C8 & C10\\
\hline
test & 61 & 30 & 23 & 13 & 0 & 0 & 0 & 0 & 0 & 0 & 0\\
\hline
\end{tabular}
\end{table}
...
\midrule
C3 & 163 & 0.190 & 0.328 & -0.761 & 0.410 & 0.539 & 0.866 & -0.809 & 0.428 & 0.304 & 0.081\\
C5 & 113 & 4.662 & 13.928 & 0.410 & 0.699 & -0.433 & 0.524 & 0.045 & 0.494 & 0.146 & 0.023\\
C2 & 56 & 8.722 & 7.219 & 0.636 & 0.839 & 0.217 & 0.836 & 1.277 & 0.462 & 0.324 & 0.116\\
C1 & 28 & 10.514 & 9.900 & 1.602 & 1.229 & -1.623 & 1.062 & 1.795 & 0.801 & 0.490 & 0.189\\
C9 & 6 & 3.104 & 5.024 & -0.187 & 0.539 & 0.173 & 0.455 & 0.093 & 0.354 & 0.157 & 0.027\\
\bottomrule
\end{tabular}}
\end{table}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{plot}\hldef{(z_test,} \hlkwc{type} \hldef{=} \hlsng{"summary"}\hldef{)}
\end{alltt}
\end{kframe}\begin{figure}[!htbp]
...
\caption{Response distributions for training and test samples under representative labels. \label{fig:cluster-old-new-summary}}\label{fig:app_cluster_old_new_summary-code}
\end{figure}
