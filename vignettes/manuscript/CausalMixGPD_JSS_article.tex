...
\Abstract{
Heavy-tailed outcomes are commonly encountered in various scientific settings where rare extreme events dominate risk, cost, and policy recommendations. Although parametric models tend to adequately capture the distributional center, they often fail to characterize extreme outcomes, and non-parametric models lack principled extrapolation in tail regions. We develop \texttt{CausalMixGPD}, an \textsf{R} package for Bayesian modeling of heavy-tailed outcomes by integrating Dirichlet process mixtures for the center and generalized Pareto distributions for the tails in a unified framework.\texttt{CausalMixGPD} allows for both unconditional and covariate-dependent models, enabling flexible models of mixture weights and components as a function of predictors. The framework is extended by allowing for separate modeling of outcome distributions for treatment groups. Posterior computations were carried out using Markov chain Monte Carlo algorithms using the Nimble package in \textsf{R}. \texttt{CausalMixGPD} is designed for estimating and predicting causal quantities, such as average and quantile treatment effects, especially extreme quantile treatment effects. Moreover, CausalMixGPD provides a clustering framework for different kernel specifications.Using real data analysis, CausalMixGPD has been shown to yield commendable performance in estimating extreme causal effects and clustering. Overall, the package offers a practical and extensible framework for heavy-tailed outcome modeling in modern statistical applications.
}
