...
\section{Model Development and Posterior Summary Calculation}
\label{sec:model}
...
<<build-fit, eval=FALSE, echo=TRUE>>=
# library(DPmixGPD)
...
#   X = X,
#   backend = "sb",        # or "crp"
#   kernel  = "gamma",     # e.g., "normal", "lognormal", "invgauss", "laplace", "cauchy", "amoroso"
#   gpd     = TRUE,
#   mcmc    = list(niter = 5000, nburnin = 1000, thin = 5, nchains = 2, seed = c(1,2))
# )
# summary(fit)
# plot(fit)
@

<<predict, eval=FALSE, echo=TRUE>>=
# pred <- predict(
#   fit,
#   newdata = Xnew,
#   type = c("density","cdf","quantile","survival"),
