---
title: "CausalMixGPD: Bayesian Conditional Dirichlet Process Mixtures with Generalized Pareto Tails and Causal Extensions"
...
if (!dir.exists(CACHE_DIR)) {
  stop(
    "Missing manuscript cache directory 'cache'. Run source(\"vignettes/manuscript/make_cache.R\") from package root or source(\"make_cache.R\") from vignettes/manuscript.",
    call. = FALSE
  )
}
cache_file <- function(...) {
  ...
  if (!file.exists(path)) {
    stop(
      sprintf(
        "Missing manuscript cache artifact: %s\nRun source(\"vignettes/manuscript/make_cache.R\") from package root or source(\"make_cache.R\") from vignettes/manuscript.",
        path
      ),
      call. = FALSE
    )
...
This article introduces \pkg{CausalMixGPD}, an \textsf{R} package that
connects Bayesian nonparametric bulk modeling with extreme-value tail
modeling, while also supporting robust causal analyses. The package
...
Dirichlet process mixture (DPM) regression. Let $k(\cdot\mid x;\theta)$
...
```{r gpdDensityFigure-code, echo=FALSE, eval=FALSE, fig=TRUE, fig.height=4}
u <- 2.0
...
```{r gpdDensityFigure-results, echo=FALSE, eval=TRUE}
...
corresponding coefficients in $\Theta$. Mixture weights $\{w_j\}$ are
generated by the DP backend (SB or CRP representation) and are
...
and tail-regression coefficients, and $\xi$ is the tail shape parameter.
...
exceedance region (Section\~\ref{subsec:gpd}). The construction is
continuous at $u(x)$ and yields a proper distribution: the bulk
contributes mass $p_u(x;\Theta)$ and the tail contributes the remaining
mass $1-p_u(x;\Theta)$.
...
```{r splicedDensityFigure-code, echo=FALSE, eval=FALSE, fig=TRUE, fig.width=7.0, fig.height=4.0}
...
```{r splicedDensityFigure-results, echo=FALSE, eval=TRUE}
...
```{r overview-mcmc-settings, echo=TRUE, eval=FALSE}
mcmc_fixed <- list(
  niter = 200,
  nburnin = 50,
  thin = 1,
...
```{r overview-fit-model-results, echo=FALSE, eval=FALSE}
fit <- read_cache_rds("fits", "overview_fit.rds")
data.frame(
  object = "fit",
  class = paste(class(fit), collapse = ", "),
  size_bytes = as.numeric(object.size(fit))
)
```
...
\code{dbeta}). Under the Chinese restaurant process (CRP) backend, the
...
```{r overview-mcmc-diagnostics-results, echo=FALSE, eval=TRUE}
overview_mcmc <- read_cache_rds("tables", "overview_mcmc.rds")
overview_summary <- overview_mcmc$overview_summary
overview_summary
```
...
```{r overview-predict-grid-results, echo=FALSE, eval=FALSE}
overview_pred <- read_cache_rds("tables", "overview_predict.rds")
overview_pred$x_new
```

```{r overview-predict-density-results, echo=FALSE, eval=FALSE}
overview_pred <- read_cache_rds("tables", "overview_predict.rds")
pdens <- overview_pred$pdens
head(as.data.frame(pdens$fit), 10)
```

```{r overview-predict-survival-results, echo=FALSE, eval=FALSE}
overview_pred <- read_cache_rds("tables", "overview_predict.rds")
psurv <- overview_pred$psurv
head(as.data.frame(psurv$fit), 10)
```

```{r overview-predict-quantile-results, echo=FALSE, eval=FALSE}
overview_pred <- read_cache_rds("tables", "overview_predict.rds")
pquant <- overview_pred$pquant
head(as.data.frame(pquant$fit), 10)
```
...
or \texttt{"crp"} backend for the DP mixture). Let
...
```{r causal-mcmc-settings, echo=TRUE, eval=FALSE}
mcmc_fixed <- list(
  niter = 200,
  nburnin = 50,
  thin = 1,
...
```{r causal-fit-model-results, echo=FALSE, eval=FALSE}
cfit <- read_cache_rds("fits", "causal_fit.rds")
cfit
```

```{r causal-ate-code, echo=TRUE, eval=FALSE}
ate_hat <- ate(cfit, level = 0.90, interval = "credible")
```

```{r causal-ate-results, echo=FALSE, eval=FALSE}
causal_derived <- read_cache_rds("tables", "causal_fit_derived.rds")
causal_derived$ate_hat
```

```{r causal-qte-code, echo=TRUE, eval=FALSE}
qte_hat <- qte(cfit, probs = c(0.50, 0.90, 0.95), level = 0.90, interval = "hpd")
```

```{r causal-qte-results, echo=FALSE, eval=FALSE}
causal_derived <- read_cache_rds("tables", "causal_fit_derived.rds")
causal_derived$qte_hat
```

```{r causal-covariate-grid, echo=TRUE, eval=FALSE}
Xnew <- df[1:5, c("x1","x2","x3")]
```

```{r causal-cate-code, echo=TRUE, eval=FALSE}
cate_hat <- cate(cfit, newdata = Xnew, type = "mean", level = 0.90)
```

```{r causal-cate-results, echo=FALSE, eval=FALSE}
causal_derived <- read_cache_rds("tables", "causal_fit_derived.rds")
causal_derived$cate_hat
```

```{r causal-cqte-code, echo=TRUE, eval=FALSE}
cqte_hat <- cqte(cfit, newdata = Xnew, probs = c(0.50, 0.90), level = 0.90)
```

```{r causal-cqte-results, echo=FALSE, eval=FALSE}
causal_derived <- read_cache_rds("tables", "causal_fit_derived.rds")
causal_derived$cqte_hat
```
...
```{r causal-predict-quantile-code, echo=TRUE, eval=FALSE}
pred_q <- predict(cfit, newdata = Xgrid, type = "quantile", p = c(0.50, 0.90))
```

```{r causal-predict-quantile-results, echo=FALSE, eval=FALSE}
causal_derived <- read_cache_rds("tables", "causal_fit_derived.rds")
pred_q <- causal_derived$pred_q
head(as.data.frame(pred_q$fit), 10)
```

```{r causal-predict-survival-code, echo=TRUE, eval=FALSE}
pred_s <- predict(cfit, newdata = Xgrid, type = "survival", y = rep(4, nrow(Xgrid)))
```

```{r causal-predict-survival-results, echo=FALSE, eval=FALSE}
causal_derived <- read_cache_rds("tables", "causal_fit_derived.rds")
pred_s <- causal_derived$pred_s
head(as.data.frame(pred_s$fit), 10)
```

```{r causal-predict-arms, echo=TRUE, eval=FALSE}
pred_q_trt <- attr(pred_q, "trt")
pred_q_con <- attr(pred_q, "con")
```

```{r causal-predict-arms-results, echo=FALSE, eval=FALSE}
causal_derived <- read_cache_rds("tables", "causal_fit_derived.rds")
pred_q <- causal_derived$pred_q
pred_q_trt <- attr(pred_q, "trt")
pred_q_con <- attr(pred_q, "con")
head(as.data.frame(pred_q_trt$fit), 10)
head(as.data.frame(pred_q_con$fit), 10)
```
...
We use the \code{Boston} housing data (\pkg{MASS}), treating each census
...
```{r app_cluster_fit, echo=FALSE, message=FALSE, warning=FALSE, results='hide', eval=TRUE}
obj <- read_cache_rds("fits", "app_cluster_fit.rds")
list2env(obj, envir = environment())
```

```{r app_cluster_fit-code, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
fit_clust <- dpmix(
...
```{r app_cluster_fit-results, echo=FALSE, eval=TRUE}
data.frame(
  sample = c("train", "new"),
...
```{r app_cluster_newdata_allocation, echo=FALSE, message=FALSE, warning=FALSE, results='hide', eval=TRUE}
obj <- read_cache_rds("tables", "app_cluster_newdata_allocation.rds")
list2env(obj, envir = environment())
```

```{r app_cluster_newdata_allocation-code, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
cluster_alloc_new <- allocation(fit_clust, newdata = newdata_cluster)
```

```{r app_cluster_newdata_allocation-results, echo=FALSE, eval=TRUE}
train_cluster_counts
new_cluster_counts
cluster_certainty
```

```{r app_cluster_newdata_barplot-code, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE, fig.width=7, fig.height=5}
...
```{r app_cluster_newdata_barplot-results, echo=FALSE, eval=TRUE}
knitr::include_graphics(cache_file("figs", "app_cluster_old_new_barplot.png"))
```
...
(GPD) tail. In contrast to Section\~\ref{sec:app-clustering}, where the
...
```{r app2-setup, echo=FALSE, message=FALSE, warning=FALSE, results='hide', eval=TRUE}
...
```{r app2-fit, echo=FALSE, message=FALSE, warning=FALSE, results='hide', eval=TRUE}
obj <- read_cache_rds("fits", "app2_fit.rds")
list2env(obj, envir = environment())
```

```{r app2-mcmc-settings, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
mcmc_fixed <- list(
  niter = 120,
  nburnin = 40,
  thin = 1,
...
```{r app2-fit-code, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
fit <- dpmgpd(
  x = y,
  X = X,
  treat = A,
...
```{r app2-fit-results, echo=FALSE, eval=TRUE}
data.frame(
...
```{r app2-taus, echo=FALSE, message=FALSE, warning=FALSE, results='hide', eval=TRUE}
taus <- read_cache_rds("tables", "app2_ate_qte.rds")$taus
```

```{r app2-ate-call, echo=TRUE, message=FALSE, warning=FALSE, results='hide', eval=TRUE}
ate_fit <- ate(fit, show_progress = FALSE)
```

```{r app2-ate-results, echo=FALSE, eval=TRUE}
ate_fit$ate$fit
```

```{r app2-qte-call, echo=TRUE, message=FALSE, warning=FALSE, results='hide', eval=TRUE}
qte_fit <- qte(fit, probs = taus, show_progress = FALSE)
```

```{r app2-qte-results, echo=FALSE, eval=TRUE}
qte_fit$qte$fit
```

```{r app2-ate-plot, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE, fig.width=7, fig.height=5}
ate_effect_plot <- plot(ate_fit, type = "effect")
ate_effect_plot
```

```{r app2-qte-plot, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE, fig.width=7, fig.height=5}
qte_effect_plot <- plot(qte_fit, type = "effect")
qte_effect_plot
```

```{r app2-ate-plot-results, echo=FALSE, eval=TRUE}
knitr::include_graphics(cache_file("figs", "app2_ate_effect.png"))
```

```{r app2-qte-plot-results, echo=FALSE, eval=TRUE}
...
```{r app2-xnew, echo=FALSE, message=FALSE, warning=FALSE, results='hide', eval=TRUE}
obj <- read_cache_rds("tables", "app2_xnew.rds")
list2env(obj, envir = environment())
```

```{r app2-xnew-code, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r app2-xnew-results, echo=FALSE, eval=TRUE}
data.frame(profile_var = profile_var)
xnew
```

```{r app2-cate-call, echo=TRUE, message=FALSE, warning=FALSE, results='hide', eval=TRUE}
cate_fit <- cate(fit, newdata = xnew, show_progress = FALSE)
```

```{r app2-cate-results, echo=FALSE, eval=TRUE}
cate_fit$ate$fit
```

```{r app2-cqte-call, echo=TRUE, message=FALSE, warning=FALSE, results='hide', eval=TRUE}
cqte_fit <- cqte(fit, probs = taus, newdata = xnew, show_progress = FALSE)
```

```{r app2-cqte-results, echo=FALSE, eval=TRUE}
cqte_fit$qte$fit
```
...
```{r app2-tail, echo=FALSE, message=FALSE, warning=FALSE, results='hide', eval=TRUE}
obj <- read_cache_rds("tables", "app2_tail.rds")
list2env(obj, envir = environment())
```

```{r app2-tail-results, echo=FALSE, eval=TRUE}
xi_table
```
