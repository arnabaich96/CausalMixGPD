...
`CausalMixGPD` is designed to handle contexts where the underlying distribution of the response variable does not admit a parametric representation that captures its complexity. Many problems have heteroskedastic, skewed, or multimodal distributions that need to be modeled with non-parametric mixtures of some form, coupled with tails that are light and may require some extrapolation. The `CausalMixGPD` package provides a solution by employing a spliced distribution, where a Dirichlet process mixture (DPM) model for the core is augmented with a possible generalized Pareto distribution (GPD) tail [@EscobarWest1995; @BalkemaDeHaan1974; @Pickands1975; @Coles2001].

The one-arm workflow is a fundamental component of the package. A conditional outcome distribution is fitted first, and densities, survival functions, quantiles, means, and restricted means are extracted from the fitted distribution. The software paper provides a detailed description of the one-arm workflow that the causal and clustering extensions are based on.

This vignette provides a comprehensive guide to the workflow using a built-in simulation study data set. There are four objectives to be achieved here. Firstly, notation and the model setup will be described. Secondly, we will provide descriptions of the one-arm workflow functions and their statistical objects. Thirdly, the interface of the package will be presented using executable code. Finally, the main customization options available in the package will be discussed.
