...
if (is.null(pkg_root)) {
  stop("Could not locate package root (DESCRIPTION). Run from inside the CausalMixGPD repo.", call. = FALSE)
}

setwd(pkg_root)
message("[CausalMixGPD] Package root: ", pkg_root)
...
if (!("cqte" %in% getNamespaceExports("CausalMixGPD"))) {
...
message("[CausalMixGPD] Rendering prebuilt HTML vignettes into inst/doc/ ...")
...
message("[CausalMixGPD] Precomputing vignette artifacts into inst/extdata/ ...")
...
# -----------------------------------------------------------------------------
data("nc_posX100_p4_k3", package = "CausalMixGPD")
...
# -----------------------------------------------------------------------------
data("causal_alt_pos500_p5_k4_tail", package = "CausalMixGPD")
...
y_a <- dat$y[1:N_use]
treat_col <- if ("A" %in% names(dat)) "A" else if ("T" %in% names(dat)) "T" else NULL
if (is.null(treat_col)) {
  stop("Causal dataset must contain a treatment column named 'A' or 'T'.", call. = FALSE)
}
A_a <- dat[[treat_col]][1:N_use]
